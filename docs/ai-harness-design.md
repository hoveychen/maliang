...
qa_patrol.py（自主巡检策略）─────────┤
naming_e2e / voice_regression …──────┤            ← 脚本层：确定性整链回归
        pilot_cli.py（一发式命令）    │
...
存量脚本经 naming_e2e re-export 兼容，不用重写。
...
- **qa_patrol.py**：无人值守巡检。状态感知加权随机策略自主玩 N 分钟；异常哨兵
  （回包错误/ws 掉线/npc 死节点/过场卡 30s/THINKING 卡 60s/连接丢失）；产出 report.md+轨迹。
  缺省不主动造物（别往 PROD 世界乱造），`--allow-create` 才开。
