...
- **白雪 gate 位**：**与七矮人同处森林深处操场**（簇北侧前方，降生 tile = `(30, 82)`，面向矮人）。
  孩子走进森林、在操场遇见白雪即搭话触发。
  **⚠️ 不放村广场（2026-07-21 老板拍板，P6 harness 坐实的 blocker）**：数数游戏是纯 tap 多演员玩法，
  需白雪 + 七矮人**同屏**；演出计数阶段相机走 `overview`（`world.gd compute_overview_cam` 取全体 actor
  中心并缩放适配）。若 gate 远在村广场（`z≈24`）而矮人在 `z≈86`，overview 会横跨 ~60 格把矮人缩成芝麻、
  点不到——P6 实机坐实：触发后画面只框住白雪、七矮人全在画外，HUD「数矮人:0」却无矮人可点。故 gate 必须与
...
**⚠️ 相机必须 overview（不能 focus 单人）**：tap 命中靠屏幕投影（`world.gd _stage_tapped_actor` 对
命中位做 3D 拾取）——演员在画外就点不到。数数是多演员玩法，计数两关全程相机走 `stage.camera.overview()`
（`compute_overview_cam` 取全体 actor 中心缩放适配），把白雪 + 七矮人整群框进画面；只在开场白雪自我介绍
和收场道谢时 `focus(snow)`。**这与 §2「gate 与七矮人同处操场」是同一枚硬币的两面**：gate 远离矮人时
overview 会把矮人缩成芝麻，同样点不到。

**零挫败保证**：无倒计时、无 `hud.countdown`、无失败分支；重复点只温柔提醒不扣分；
中途走开 → 舞台 abort（`status!=='done'`）→ 幕首恢复，重触发从头玩，不发重复奖。

**✅ P6 已坐实（2026-07-21 harness 沙箱）**：本地 server + 沙箱 seed 白雪+七矮人 + 桌面客户端，对白雪说
"讲个故事"→路由 start_story→演出开演、HUD「数矮人:0」（tap 计数阶段激活）——触发链路通。**但首轮暴露 blocker**：
白雪原在村广场、七矮人在森林深处，相机 focus(snow) 锁村广场、矮人全在画外点不到。修复 = gate 挪到操场
（§2）+ 计数阶段 camera.overview()（本节）。修复后重验：七矮人同屏、逐个 tap 计数真走到 7、盛碗、
`stage.end`→盖章→尾声→入住。
