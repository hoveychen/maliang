...
**第二关·一人一个碗（一一对应）**（`prop.create` 七个碗 + `on('enter', player, region_i)`）：
- 点点："吃饭啦！七个小矮人，要给每人一个小碗——一个都不能少哦。"
- 每个矮人脚下画一个判定圈（`stage.region`）。孩子走到某个**还没配碗**的矮人身边（`enter`）→
  在那个矮人面前 `prop.place` 一个碗 → 计数器 +1 → 点点报数 → 那个矮人道谢。
- 走到**已配碗**的矮人：点点"他已经有碗啦～"（不重复计数）。
- 七个碗都配上 → 点点："每个小矮人都有碗啦，七个人七个碗，正正好！"（第二关过）。

**收场**：白雪道谢（"谢谢你帮我把家收拾好、把小矮人都数清楚！"）→ `stage.end({ praise: '数得真棒！' })`。
`stage.end` 一发即"演赢"（`StageRunResult.status==='done'`）→ 触发 §5 的 performReward 发盖章。

**零挫败保证**：无倒计时、无 `hud.countdown`、无失败分支；点错/走错只温柔提醒不扣分；
中途走开 → 舞台 abort（`status!=='done'`）→ 幕首恢复，重触发从头玩，不发重复奖。

**⚠️ 关键风险（P6 必验）**：数数游戏靠**孩子在故事演出期间能点/能走**。`tap` 是客户端本地探测
（观演态点角色即上行，`stage_agent.on_local_tap`）——大概率可用。**第二关的走位（`enter` 判定）
依赖"故事演出期间玩家能自由移动 avatar"**，这条要在 P6 用 harness/真机确认。若演出期玩家被锁走位，
第二关降级为**纯 tap 版一一对应**（点一个矮人再点一个碗配对，全 tap，不依赖走位），数学目标不变。
