...
import { AVATAR_FORBIDDEN_DESC, avatarOptionsByCategory, composeAvatarDesc, findAvatarOption } from '../avatar_options.ts';
...
硬规则（每一条都必须遵守）：
1. 双手空着——绝不写 抱着/拿着/手持/举着/捧着/牵着 任何东西，双手自然垂在身边或叉腰。
2. 小朋友的喜好一律转译成【穿戴元素】：喜欢恐龙→恐龙图案连帽衫或带小角的帽子，不是抱恐龙玩偶；喜欢踢球→足球图案T恤或运动鞋，不是抱着球。
3. 只描述这一个孩子本身的外观（发型、衣服、颜色、图案、配饰、表情），不出现第二个角色、宠物、玩具、背景或道具。
4. 小朋友的原话优先（「会发光的头发」就写会发光的头发），比选项词更宝贵。
5. 2~3 句以内，具体、可画，不写画风/构图/背景——服务端会统一追加。
只输出描述文字本身，不要 JSON、不要引号、不要解释。`;
    ...
    // 违禁措辞（持物）→ 带着违规原文重试一次；仍违规或 LLM 失败 → 确定性兜底模板
    try {
      let desc = (await this.#client.chatText(this.#model, [
        { role: 'system', content: system }, { role: 'user', content: user },
      ])).trim();
      if (AVATAR_FORBIDDEN_DESC.test(desc)) {
        desc = (await this.#client.chatText(this.#model, [
          { role: 'system', content: system },
          { role: 'user', content: `${user}\n\n上一稿违反了硬规则1（出现了持物措辞）：「${desc}」。重写：把手里的东西改成衣服上的图案或配饰，双手必须空着。` },
        ])).trim();
      }
      if (desc && !AVATAR_FORBIDDEN_DESC.test(desc)) return desc;
    } catch {
      // 走兜底
    }
...
硬规则与原描述相同：双手空着绝不持物（喜好转译为穿戴元素）、只有这一个孩子、不写画风/构图/背景。
只输出修改后的完整描述文字，不要 JSON、不要引号、不要解释。`;
    ...
    try {
      ...
      if (AVATAR_FORBIDDEN_DESC.test(desc)) {
        desc = (await this.#client.chatText(this.#model, [
          { role: 'system', content: system },
          { role: 'user', content: `${user}\n\n上一稿违反了硬规则（出现了持物措辞）：「${desc}」。重写：把手里的东西改成衣服上的图案或配饰，双手必须空着。` },
        ])).trim();
      }
      if (desc && !AVATAR_FORBIDDEN_DESC.test(desc)) return desc;
    } catch {
    ...
    // LLM 失败：原描述追加修改要求（不丢小朋友的话，生图仍能受益）
