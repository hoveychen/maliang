...
- 绝不包含暴力、恐怖、成人内容。`;
  ...
  async guideCreation(state: CreationState, childInput: string): Promise<GuideCreationResult> {
    ...
    const asked = state.askedCategories.length > 0 ? state.askedCategories.join('、') : '（无）';
    const system = `你是幼儿游戏里温柔的小神仙，正在按小朋友的想法一步步造一个新伙伴。
已知道的属性：${known}。
已问过的类别：${asked}。不要重复问已经问过的类别。
你要么再问一个还不知道的属性（一次只问一个，配 2-4 个选项图标），要么信息够了就开始造。
可选的属性类别与图标（选项用 id）：
${catLines}
名字(name)没有图标：想问名字时 category 填 "name"、optionIds 留空，小朋友会用语音说。
小朋友这轮说的话是在回答你上一轮的问题（若有），按那个问题的类别解读并填进 updatedAttrs。
...
updatedAttrs 只填这轮从小朋友输入里新解析出的属性（没有就省略字段）。绝不包含暴力、恐怖、武器、成人内容。`;
    // 上一轮问句以 assistant 消息回放，LLM 才有上下文解读答案（如自由语音「毛毛」是在答名字）。
    const content = await this.#client.chatText(
      this.#model,
      [
        { role: 'system', content: system },
        ...(state.lastQuestion ? [{ role: 'assistant' as const, content: state.lastQuestion }] : []),
...
严格只输出 JSON：{"replyText":"你要对小朋友说的话(中文,温暖童趣,≤两句,若在问就把问题和选项自然念出来)","done":true或false,"description":"done时:把所有属性汇成一句给设计师的中文描述(如『一个红色小小的风车，会转』)","question":"done=false时的问题","category":"done=false时问的类别","optionIds":["done=false时的选项id"],"updatedAttrs":{"kind":"","color":"","size":"","motion":""}}
updatedAttrs 只填这轮从小朋友输入里新解析出的属性（没有就省略字段）。绝不包含暴力、恐怖、武器、成人内容。`;
    const content = await this.#client.chatText(
      this.#model,
      [
