...
import { buildSpritePrompt, buildIconPrompt } from './sprite_style.ts';
...
export class OpenRouterImageAdapter implements ImageAdapter {
  ...
  async generateIcon(visualDescription: string): Promise<ImageBlob> {
    return this.#client.chatImage(this.#model, buildIconPrompt(visualDescription));
  }
}
