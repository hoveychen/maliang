...
// ---- 资产对拍：renderRef ↔ 打包贴图（P2） ----

test('每个 sticker:<name> renderRef 都有对应的 assets/stickers/<name>.webp（真 WebP）', () => {
  const dir = join(dirname(fileURLToPath(import.meta.url)), '..', '..', 'assets', 'stickers');
  const stickers = BUILTIN_ITEMS.filter((d) => d.mount === 'edge');
  assert.ok(stickers.length >= 12, '内置贴纸至少 12 张');
  for (const d of stickers) {
    const name = d.renderRef.split(':')[1]!;
    const file = join(dir, `${name}.webp`);
    assert.ok(existsSync(file), `${d.id} 缺贴图 ${file}`);
    const head = readFileSync(file).subarray(0, 12);
    assert.equal(head.subarray(0, 4).toString('ascii'), 'RIFF', `${name}.webp 不是 RIFF`);
    assert.equal(head.subarray(8, 12).toString('ascii'), 'WEBP', `${name}.webp 不是 WebP`);
  }
});
