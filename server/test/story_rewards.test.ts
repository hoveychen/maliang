...
test('幕 3 build：0 花也能拼（免花不扣不退），落成即结算盖章+「砖房」贴纸', async () => {
  ...
  assert.equal(store.getStoryProgress(W, P).books[BOOK]!.chapter, 3);
});

// ── 尾声入住 + 重看跳奖 ─────────────────────────────────────────────────

test('尾声演完整册完结：小猪入住（resident+委托链+供给面放行），狼不入住；重看不再发奖', async () => {
  const adapters = createMockAdapters();
  const store = await seededStore(adapters);
  const h = harness(store, adapters);
  // 走完三幕互动
  ...
  assert.equal(store.getCharacter(W, storyCharacterId(BOOK, 'wolf'))!.storyRole?.resident, false, '狼不入住');
  // 供给面放行：入住的小猪能派活了
...
test('settleStoryResidency 幂等：二次结算不重复翻转', async () => {
  const adapters = createMockAdapters();
  const store = await seededStore(adapters);
  assert.equal(settleStoryResidency(W, THREE_PIGS, store, adapters.llm).length, 3);
