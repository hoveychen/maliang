...
import { WISHES, IDLE_DOING, ERRAND_LEAKS, WISH_ABILITIES, wishFor } from '../src/wishes.ts';
...
test('玩法全发现后回落 errand:idle 混合——不再勾玩法，但拉力不干涸（M1 §2.3）', async () => {
  const store = seedWorld();
  ...
  for (const w of wishesOf()! as (WishPush & { source?: string })[]) {
    // 无链的存量村民：按 characterId+日期稳定落到通用跑腿引子或纯氛围，两者都合法、但绝不哑
    if (w.source === 'errand') assert.deepEqual(w.lines, ERRAND_LEAKS);
    else {
      assert.equal(w.source, undefined, '纯氛围不算供给信号，不该带 source');
      assert.deepEqual(w.lines, IDLE_DOING, '心愿池空且没轮到跑腿引子时回落 IDLE_DOING');
    }
  }
});
