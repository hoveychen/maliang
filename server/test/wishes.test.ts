...
import { ERRAND_LEAKS, WISHES, WISH_ABILITIES, IDLE_DOING, wishFor, pickLeak, pickThanks } from '../src/wishes.ts';
...
test('漏话不含广告腔——没有「你可以」「要不要」「告诉我」这类招揽话术', () => {
  ...
  for (const line of ERRAND_LEAKS) {
    for (const b of banned) {
      assert.ok(!line.includes(b), `跑腿引子「${line}」含招揽话术「${b}」`);
    }
  }
});
...
test('玩法全被发现后不再有心愿——回落 errand:idle 混合，绝不哑（M1 §2.3）', () => {
  const all = [...WISH_ABILITIES];
  assert.equal(wishFor('npc-abc', all), null);
  const leak = pickLeak('npc-abc', all, () => 0);
  assert.ok(
    IDLE_DOING.includes(leak) || ERRAND_LEAKS.includes(leak),
    `池耗尽时应回落氛围自语或跑腿引子，却拿到「${leak}」`,
  );
});
