...
test('s1-P1 additive 只加不改：孩子已编辑过的同名场景【地形】不被模板覆盖（保住地形编辑）', () => {
  const s = seedTemplate();
  const wa = s.getOrCreateMyWorld('alice');
  // 孩子在自己世界里编辑过主场景（terrainAsset=h_<wa>，模拟摆放/改地形——per-world 层）
  registerScene(s, wa, 'village_forest', [GRANDMA_POI]);

  // 模板登记一份同名场景（terrainAsset=h_template）+ bump
  registerScene(s, TEMPLATE_WORLD_ID, 'village_forest', [GRANDMA_POI]);
  s.bumpTemplateVersion();

  s.getOrCreateMyWorld('alice'); // 触发迁移
  const scene = s.getScene(wa, 'village_forest')!;
  // additive 不覆盖已存在场景：孩子的地形（per-world 层）保留，不被模板的 h_template 冲掉
  assert.equal(scene.terrainAsset, `h_${wa}`, 'additive 不覆盖已存在场景的地形');
  assert.ok(s.getSceneTerrain(wa, 'village_forest'), '孩子的地形 blob 仍在');
  // base+overlay P2：pois 是作者字段（无孩子编辑入口），读时一律取自 template base
  assert.equal(scene.pois[0]!.trigger, 'poi_grandma', 'pois 走 template base（P2 作者字段）');
});
