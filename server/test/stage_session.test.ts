...
test('规则订阅: on(near) 不下发客户端 watch，服务端对复制位置边沿求值 → 触发 end', async () => {
  ...
  await a.say({ type: 'world_info', worldId: 'w1', playerId: 'p1' });
  const done = stages.startStage('w1', {
    // 门闩顺序要紧：hud_score 放在 on('near') 之后——host 按序处理 worker 消息，看到 hud_score 广播
    // 即证明其前的 subscribe 已登记。反过来（hud 在前）只证明 hud 处理了、不保证订阅到位，会漏掉随后
    // 同步连发的 updatePositions（enter 测试同源，见下方 296 行注释）。actors[0]=a1(村民), actors[1]=p1(玩家)；靠近阈值 5。
    code: `stage.on('near', stage.actors[0], stage.actors[1], 5, () => stage.end({ caught: true })); stage.hud.score('ready','');`,
    actors: ACTORS,
    timeoutMs: 5000, // 事件驱动应在 ms 内收场；给上限，回归时快速失败而非挂满 10min
  });
...
test('规则订阅: near 边沿——贴着不重复触发，离开后重新靠近再触发', async () => {
  ...
  await a.say({ type: 'world_info', worldId: 'w1', playerId: 'p1' });
  const done = stages.startStage('w1', {
    // hud_score 当订阅门闩，须放订阅之后（见上一个 near 测试的顺序说明）；脚本内计数：第 2 次靠近才收场。
    code: `let n = 0; stage.on('near', stage.actors[0], stage.actors[1], 5, () => { n++; if (n >= 2) stage.end({ n }); }); stage.hud.score('ready','');`,
    actors: ACTORS,
    timeoutMs: 5000, // 回归时快速失败而非挂满 10min
  });
  await waitFor(() => a.ofType('stage_cmd').some((m) => m.op === 'hud_score'));
