...
import { FAIRY_VOICE } from '../src/voice_catalog.ts';
...
test('存量迁移：老库里叫「小神仙」的仙子，重开后改名点点并换上新人设（幂等，不动非仙子）', () => {
  ...
  try {
    ...
    // 模拟迁移前的存量数据：旧名字 + 旧人设 + 旧音色 + 带着两个永远被剥夺的能力
    const old = seedFairy('default');
    old.id = 'old-fairy';
    old.name = '小神仙';
    old.personality = '温柔的小神仙，能按小朋友的想法创造新伙伴。';
    old.voiceId = 'zh-CN-XiaoyiNeural'; // 旧音色（少女音）
    ...
    assert.equal(f.voiceId, FAIRY_VOICE, '音色应迁移到点点音色（动态对话 TTS 与预制台词一致）');
...
test('存量迁移：老库点点缺 create_prop（历史窗口期建库）重开后被补齐——造物入口不能是死的', () => {
  // 实证来源：本地一个历史遗留开发库里的点点 abilities 恰好缺 create_prop，
  // 「点点，帮我造一个小房子」永远进不了造物/拼装。#migrateFairyAbilities 的 REQUIRED
  // 只列了「后续新增」的能力，漏了 create_prop 这类「一直都该有」的——这里锁死全量看家本领。
  const dir = join(tmpdir(), `maliang-fairy-abilities-${process.hrtime.bigint()}`);
  rmSync(dir, { recursive: true, force: true });
  try {
    const store = new WorldStore(dir);
    store.createWorld('default');
    const old = seedFairy('default');
    old.id = 'old-fairy';
    old.abilities = ['create_character', 'create_sticker', 'play_game', 'guide_to', 'guide_stop']; // 缺 create_prop
    store.addCharacter(old);

    const store2 = new WorldStore(dir); // 重开库 → 启动迁移
    const f = store2.getCharacter('default', 'old-fairy')!;
    for (const a of ['create_character', 'create_prop', 'create_sticker', 'play_game', 'guide_to', 'guide_stop']) {
      assert.ok(f.abilities.includes(a), `迁移后点点必须有 ${a}`);
    }
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
