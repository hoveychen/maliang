...
// template 世界（由 default 内容提升）+ cloneWorldInstances（只复制实例放置，def 引用不变）+
...
/** 给 default 种最小内容（一个 story 角色 + 一个村民），供 template 提升与克隆。 */
function seedDefault(): WorldStore {
...
test('P2 cloneWorldInstances：复制实例放置到目标世界（id/defId/位置/状态原样，只换 worldId）', () => {
  const s = seedDefault();
  s.cloneWorldInstances('default', 'dst');
...
// ── ensureTemplateWorld：由现 default 内容提升为模板 ──────────────────────────────

test('P2 ensureTemplateWorld：把 default 内容提升为 template（幂等，不重复克隆）', () => {
  const s = seedDefault();
  s.ensureTemplateWorld();
  assert.ok(s.worldExists('template'));
  assert.equal(s.listCharacters('template').length, 2, 'template 拿到 default 的两个实例');
  // 幂等：再调一次不翻倍
  s.ensureTemplateWorld();
  assert.equal(s.listCharacters('template').length, 2, '重复调用不重复克隆');
});
...
test('P2 getOrCreateMyWorld：template 无点点时用注入的 makeFairy 补种；有则不重复种', () => {
  const s = seedDefault(); // default 无仙子
  let calls = 0;
...
// ── ④ template 不接客：GET /worlds/template 不自动建 ────────────────────────────
...
test('P2 ⑤ 复合 PK 迁移幂等：持久库重开不炸、老库角色存活、克隆后两世界同 id 各一份', () => {
  ...
  try {
    ...
    // 首开：建 default + 种角色（走真实读写路径，落进复合 PK 的库）
    {
      const s = new WorldStore(dir);
      s.createWorld('default');
      s.saveCharacter(storyPig('default'));
      // 提升模板 + 建两个玩家世界（同 story id 各一份，复合 PK 才装得下）
      const wa = s.getOrCreateMyWorld('alice');
      const wb = s.getOrCreateMyWorld('bob');
