...
// s1-hood-activate P1：主场景 village_forest 的 poi_grandma——guide_to 引路终点，planGuide 从
// listScenes().pois 取其 tile。场景层须随克隆/迁移传到玩家世界，否则引路 return null。
const GRANDMA_POI: ScenePoi = { tile: [66, 63], radius: 14, trigger: 'poi_grandma', name: '外婆家', aliases: ['外婆', '奶奶家', '小屋'] };

/** 往 template 登记一张主场景（含 poi_grandma + 100 格地形 blob）。返回地形字节供比对。 */
function seedTemplateScene(s: WorldStore, sceneId = 'village_forest'): Uint8Array {
  const bytes = encodeTerrain(emptyTerrain(100));
  s.upsertScene({
    worldId: TEMPLATE_WORLD_ID, sceneId, name: '林边村庄', terrainAsset: 'hashVF', gridTiles: 100,
    pois: [GRANDMA_POI], portals: [], terrainVersion: 1,
  });
  s.setSceneTerrain(TEMPLATE_WORLD_ID, sceneId, bytes, 1);
  return bytes;
}
...
// ── s1-hood-activate P1：场景层随克隆传播（《小红帽》guide_to poi_grandma 硬依赖 scenes.pois）──

test('s1-P1 cloneWorldInstances：模板的场景（含 POI + 地形 blob）随克隆整行复制进目标世界', () => {
  const s = seedTemplate();
  const bytes = seedTemplateScene(s);
  s.cloneWorldInstances(TEMPLATE_WORLD_ID, 'dst');
  const scene = s.getScene('dst', 'village_forest');
  assert.ok(scene, '目标世界应有克隆过来的主场景');
  assert.equal(scene!.gridTiles, 100, 'grid 原样');
  assert.deepEqual(scene!.pois, [GRANDMA_POI], 'POI（含 poi_grandma tile）原样复制——引路才有目标');
  const terrain = s.getSceneTerrain('dst', 'village_forest');
  assert.ok(terrain, '地形 blob 也随克隆复制');
  assert.deepEqual(new Uint8Array(terrain!.bytes), new Uint8Array(bytes), '地形字节逐字节一致');
});

test('s1-P1 getOrCreateMyWorld / 沙箱：新世界从模板克隆时即拿到主场景 + poi_grandma', () => {
  const s = seedTemplate();
  seedTemplateScene(s);
  const wid = s.getOrCreateMyWorld('alice');
  assert.ok(s.getScene(wid, 'village_forest'), '每人一世界克隆时带主场景');
  const sandbox = s.createSandboxWorld();
  assert.ok(s.getScene(sandbox, 'village_forest'), '沙箱世界克隆时带主场景');
  assert.equal(s.getScene(sandbox, 'village_forest')!.pois[0]!.trigger, 'poi_grandma');
});
