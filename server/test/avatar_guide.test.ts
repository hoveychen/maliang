...
test('describeAvatar：纯外观描述，双手空着，图案转穿戴，无持物措辞', async () => {
  const llm = createMockAdapters().llm;
  ...
  assert.ok(!AVATAR_FORBIDDEN_DESC.test(desc), `描述不得有持物措辞：${desc}`);
...
test('composeAvatarDesc：属性缺省也能出可用描述（降级链兜底）', () => {
  ...
  assert.ok(!AVATAR_FORBIDDEN_DESC.test(empty));
