...
test('generateCharacterAnimation: 同批原片打两档图集 → 底座 8fps + 高保真 24fps，各自独立入库', async () => {
  const store = new WorldStore();
  const sprite = putSprite(store);

  // fps 敏感的假打包器：按 fps 回不同 meta/字节，好验两档确实各打一次、各自入库。
  const fpsSeen: number[] = [];
  const perFps: ToSpriteSheet = async (_clips, fps) => {
    fpsSeen.push(fps);
    return {
      atlas: { bytes: Uint8Array.from([fps]), mime: 'image/png' }, // 不同 fps → 不同字节 → 不同 hash
      meta: { ...META, fps },
    };
  };
  await generateCharacterAnimation(createMockAdapters(), store, sprite, perFps);

  assert.deepEqual(fpsSeen, [LO_FPS, HI_FPS], '应先打底座 8fps 再打高保真 24fps，同批 clips');

  const rec = store.getSpriteAnim(sprite)!;
  assert.equal(rec.meta?.fps, LO_FPS, 'animAsset/meta 是底座档（8fps）');
  assert.ok(rec.hiAnimAsset, '应记下高保真档图集 hash');
  assert.equal(rec.hiMeta?.fps, HI_FPS, 'hiMeta 是高保真档（24fps）');
  assert.notEqual(rec.hiAnimAsset, rec.animAsset, '两档是各自独立的资产');
  assert.ok(store.getAsset(rec.hiAnimAsset!), '高保真图集资产应可取回');
  // 打包管线版本要落到当前值（否则每次开机反复 repack）。
  assert.equal(rec.packVersion, SPRITE_PACK_VERSION);
});

test('repackFromStoredClips: 从存量原片零成本补出两档（含高保真 24fps）', async () => {
  const store = new WorldStore();
  const sprite = putSprite(store);
  // 先用只出底座档的老假打包器造记录（模拟 v2 存量：有原片、无 hi 字段）。
  const loOnly: ToSpriteSheet = async () => ({
    atlas: { bytes: Uint8Array.from([1, 1]), mime: 'image/png' }, meta: META,
  });
  await generateCharacterAnimation(createMockAdapters(), store, sprite, loOnly);

  const fpsSeen: number[] = [];
  const perFps: ToSpriteSheet = async (_clips, fps) => {
    fpsSeen.push(fps);
    return { atlas: { bytes: Uint8Array.from([fps]), mime: 'image/png' }, meta: { ...META, fps } };
  };
  assert.equal(await repackFromStoredClips(store, sprite, perFps), true);
  assert.deepEqual(fpsSeen, [LO_FPS, HI_FPS], 'repack 也打两档');

  const rec = store.getSpriteAnim(sprite)!;
  assert.equal(rec.meta?.fps, LO_FPS);
  assert.ok(rec.hiAnimAsset, 'repack 后应有高保真档');
  assert.equal(rec.hiMeta?.fps, HI_FPS);
});
