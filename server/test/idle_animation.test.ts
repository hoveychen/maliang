...
/** 三段图集的 meta（4+3+3=10 帧，4×4 网格容得下）。 */
const META: SpriteSheetMeta = {
  cols: 4, rows: 3, frameCount: 10, fps: 8, cellW: 20, cellH: 30, width: 80, height: 90,
  clips: { idle: { start: 0, count: 4 }, moving: { start: 4, count: 3 }, talking: { start: 7, count: 3 } },
};
...
function setReadyV2(store: WorldStore, sprite: string, animAsset: string): void {
  store.setSpriteAnimReady(sprite, animAsset, META, {
    ...
    clipVideos: { idle: 'vi', moving: 'vm', talking: 'vt' },
  });
}

test('generateCharacterAnimation: 三段并发生成 → 一张图集入库，记录带 version + 三段原片 hash', async () => {
  const store = new WorldStore();
  const sprite = putSprite(store);

  // 拦住转换缝，看清楚交给它的到底是不是「idle/moving/talking 三段」。
  let handed: string[] = [];
  ...
  assert.deepEqual(handed, ['idle', 'moving', 'talking'], '应把三段一起交给打包器（段序即图集段序）');
  ...
  assert.ok(vids, '应记下三段原片的资产 hash');
  ...
  assert.equal(
    new Set(Object.values(vids!)).size, 3,
    '三段原片应是三个不同的资产（内容不同 → hash 不同）',
  );
});
...
test('repackFromStoredClips: 拿已存原片重打图集，全程不碰视频生成（零成本）', async () => {
  ...
  const repacked: ToSpriteSheet = async (clips) => {
    assert.deepEqual(clips.map((c) => c.name), ['idle', 'moving', 'talking'], '应从库里取回三段原片');
...
test('triggerCharacterAnimation: 老的单段 v1 ready → 升级重生成（否则老角色永远只有 idle）', async () => {
  ...
  const spy: ToSpriteSheet = async (clips) => {
    called = true;
    assert.equal(clips.length, 3, '升级应生成三段');
...
test('sprite-anim 持久化：重启后 ready 保留（含 version/原片 hash）、pending 转 failed', () => {
  ...
  s1.setSpriteAnimReady('ready1', 'atlasA', META, {
    version: SPRITE_ANIM_VERSION,
    clipVideos: { idle: 'v1', moving: 'v2', talking: 'v3' },
  });
  ...
  assert.deepEqual(rec?.clipVideos, { idle: 'v1', moving: 'v2', talking: 'v3' }, '原片 hash 要落盘');
