...
// 幂等性：medium 判定=scale 1.0 曾与「未标定」歧义，重跑会反复重分类（非幂等）。
// 用 appearance.size 标记位后，medium 也粘住：第二次跑应 skip。
test('/admin/calibrate-size：medium 也粘住，重跑幂等（不再反复重分类）', async () => {
  const store = new WorldStore();
  const app = await buildServer({ adapters: createMockAdapters(), store });
  process.env.MALIANG_ADMIN_TOKEN = 'sesame';
  try {
    store.createWorld('w1');
    store.addCharacter(char('w1', 'mid', 'a round friendly blob', 1.0)); // 无体型词→medium→scale 1.0
    const url = '/admin/calibrate-size?world=w1';
    const H = { 'x-admin-token': 'sesame' };

    // 第一次：medium 被标定，scale 保持 1.0，但落下 appearance.size 标记
    const r1 = (await app.inject({ method: 'POST', url, headers: H })).json() as { results: { id: string; size?: string; skipped?: string }[] };
    assert.equal(r1.results.find((r) => r.id === 'mid')?.size, 'medium', '首次判为 medium');
    assert.equal(store.getCharacter('w1', 'mid')?.appearance.scale, 1.0);
    assert.equal(store.getCharacter('w1', 'mid')?.appearance.size, 'medium', '标记位已落');

    // 第二次：应因已标记而 skip（幂等），不再重分类
    const r2 = (await app.inject({ method: 'POST', url, headers: H })).json() as { results: { id: string; size?: string; skipped?: string }[] };
    assert.equal(r2.results.find((r) => r.id === 'mid')?.skipped, 'already calibrated', 'medium 第二次应 skip');
  } finally {
    delete process.env.MALIANG_ADMIN_TOKEN;
    await app.close();
  }
