...
import { seedFairy, startStoryAsync, newVoiceSession } from '../src/server.ts';
...
import { STORY_BOOKS } from '../src/story_books.ts';
...
test('visit 委托一物化就自动带引路计划——不必回头对点点说「带我去外婆家」（P2 降门槛）', async () => {
  const store = await seedHoodWorld();
  const stages = new StageDirector(new WorldHub());
  const stories = new StoryDirector(store, async () => ({ status: 'done' }), STORY_BOOKS);
  const sent: Record<string, unknown>[] = [];
  const socket = { send: (s: string) => sent.push(JSON.parse(s)) };
  const session = newVoiceSession();
  session.playerId = 'p1';
  session.currentScene = VF;
  session.clientTts = true;
  // 从幕首开演《小红帽》——幕1 是 visit 外婆家，演出收场即物化委托并递出
  await startStoryAsync(
    socket, session, 'w1', storyCharacterId('hood', 'red_hood'), 'hood',
    '好呀，我们这就走！', createMockAdapters(), store, new WorldHub(), stages, stories,
  );
  await new Promise((r) => setImmediate(r)); // trigger 不被 await：等 mock 演出收场→offerStoryTask
  const offer = sent.find((m) => m['type'] === 'task_offer') as { task?: { type?: string }; guide?: { targetName?: string; targetTile?: unknown } } | undefined;
  assert.ok(offer, '演出收场后递出 visit 委托');
  assert.equal(offer!.task?.type, 'visit');
  assert.ok(offer!.guide, 'visit 委托应自动带引路计划（点点飞前面带路），不必孩子回头找点点');
  assert.equal(offer!.guide!.targetName, '外婆家');
  assert.deepEqual(offer!.guide!.targetTile, { tileX: 66, tileY: 63 });
});
