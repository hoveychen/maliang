...
// 零污染 default/template。删走既有 admin DELETE（级联）。这里在服务端权威层证「隔离」：沙箱里入住
// 只翻自己那份，default/template/另一沙箱不受牵连；改共享定义一次全沙箱都变（克隆引用共享 def）。
...
async function freshServer(t: { after: (fn: () => unknown) => void }): Promise<{ app: Awaited<ReturnType<typeof buildServer>>; store: WorldStore }> {
  ...
  store.saveCharacter(storyPig('default'));
...
test('P4 沙箱端点：token 门禁 + 从 template 克隆出全新沙箱（含 story 角色 + 点点）', async (t) => {
  ...
  // 含 template（由 default 提升）的 story 角色
  ...
  // 没污染 default（default 里没多出沙箱）——沙箱是独立世界
  assert.notEqual(body.id, 'default');
});
...
test('P4 隔离：沙箱里入住只翻自己那份，default/template/另一沙箱都不受牵连', async (t) => {
  ...
  assert.equal(store.getCharacter('default', STORY_ID)!.storyRole!.resident, false, 'default 不受牵连（零污染）');
  assert.equal(store.getCharacter('template', STORY_ID)!.storyRole!.resident, false, 'template 不受牵连');
  assert.deepEqual(store.getCharacter('default', STORY_ID)!.position, { tileX: 29, tileY: 49 }, 'default 位置不动');
});
...
test('P4 共享定义：改一次 def → 沙箱里那份也变（克隆引用共享定义，非复制）', async (t) => {
  ...
  assert.equal(store.getCharacter('default', STORY_ID)!.name, '猪二哥', 'default 也随同一份定义更新');
});

test('P4 建/删闭环：DELETE 沙箱级联清掉，default/template 完好', async (t) => {
  const { app, store } = await freshServer(t);
  const s = (await app.inject({ method: 'POST', url: '/admin/worlds/sandbox', headers: TOK })).json() as { id: string };
  ...
  // default/template 仍在，其 story 角色完好（沙箱删除不碰共享定义/别的世界）
  assert.ok(store.worldExists('default'));
  assert.ok(store.worldExists('template'));
  assert.equal(store.getCharacter('default', STORY_ID)!.name, '猪大哥', 'default 角色完好');
