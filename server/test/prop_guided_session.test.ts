...
// 多轮点选累积、done 走 createPropAsync（prop_pending + prop_created）。与造角色 guided_creation 平行。
...
test('多轮：入口给种类 → creation_reply 点颜色 → 攒够 done → prop_pending + prop_created 落库', async () => {
  const { store, fairyId, close } = await seeded();
  try {
    const session = newVoiceSession();
    const before = store.listProps('default').length;
    ...
    assert.equal(sent.some((m) => m.type === 'prop_created'), true, '造好推 prop_created');
    assert.equal(store.listProps('default').length, before + 1, '物件入库');
  } finally {
    await close();
  }
});

test('快捷路径：一句说全「变一个红色的风车」→ 首轮即造，不追问', async () => {
  const { store, fairyId, close } = await seeded();
  try {
    const session = newVoiceSession();
    const before = store.listProps('default').length;
    const sent = await ws(store, session, { type: 'voice_transcript', worldId: 'default', characterId: fairyId, transcript: '变一个红色的风车' });
    assert.equal(sent.some((m) => m.type === 'creation_prompt'), false, '一句说全不追问');
    assert.equal(sent.some((m) => m.type === 'prop_created'), true, '直接造出来');
    assert.equal(store.listProps('default').length, before + 1);
