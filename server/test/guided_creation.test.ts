...
// A2「给谁做的」：造物会话最前先问一步 recipient（docs/kids-thinking-made-for-whom.md）。测试里统一答「大家」
// ——不预填 size（保留后续正常属性追问），把暂存的入口话喂给 advanceCreation 继续。返回该步下发的消息。
...
test('缺陷②：一句说全的入口话语也要念前置话语（A2 下并入 recipient 问句，不吞掉）', async () => {
  const { store, fairyId, close } = await seeded();
  try {
    const session = newVoiceSession();
    // A2：即便一句说全，入口先问 recipient——前置话语并入 recipient 问句一次念完，绝不吞掉。
    const sent = await ws(store, session, { type: 'voice_transcript', worldId: 'default', characterId: fairyId, transcript: '我想要一只会飞的红色小猫' });
    const prompt = sent.find((m) => m.type === 'creation_prompt' && m.category === 'recipient');
    assert.ok(prompt, `入口应先下发 recipient 问句，实际消息：${sent.map((m) => m.type).join(',')}`);
    assert.ok(String(prompt!.replyText).startsWith(LEAD_IN), `前置话语应并入 recipient 问句开头，实际：${String(prompt!.replyText)}`);
  } finally {
    await close();
  }
});

test('多轮：入口→答 recipient→给类型→点颜色→攒够 done → gen_complete 入库', async () => {
  const { store, fairyId, close } = await seeded();
  try {
    const session = newVoiceSession();
    const before = store.listCharacters('default').length;
    // 入口：说「小猫」→ A2 先问 recipient（此时还没解析 kind，入口话被暂存）
    await ws(store, session, { type: 'voice_transcript', worldId: 'default', characterId: fairyId, transcript: '我想要一只小猫' });
    assert.equal(session.creation?.active, true);
    // 答 recipient「大家」→ 用暂存的入口话喂 advanceCreation → 解析出 kind，缺颜色 → 追问颜色
    await answerRecipient(store, session, fairyId);
...
test('快捷方式：一句说全 → 答 recipient 后即 done，不发属性 creation_prompt', async () => {
  const { store, fairyId, close } = await seeded();
  try {
    const session = newVoiceSession();
    const before = store.listCharacters('default').length;
    // 入口先问 recipient（会话开着）
    await ws(store, session, { type: 'voice_transcript', worldId: 'default', characterId: fairyId, transcript: '我想要一只会飞的红色小猫' });
    assert.equal(session.creation?.active, true, 'recipient 步会话开着');
    // 答 recipient → 暂存的整句喂 LLM → 一句说全直接 done，不再追问属性
    const sent = await answerRecipient(store, session, fairyId);
    assert.equal(session.creation, null, '一句说全，答完 recipient 即完成');
...
test('小红花扣费：引导造角色 done → 恰扣 1 朵花，gen_complete 带最新钱包', async () => {
  ...
  try {
    ...
    // 一句说全 → 入口问 recipient → 答 recipient → done → createCharacterAsync 扣 1 朵
    await ws(store, session, { type: 'voice_transcript', worldId: 'default', characterId: fairyId, transcript: '我想要一只会飞的红色小猫' });
    const sent = await answerRecipient(store, session, fairyId);
...
test('会话进行中普通语音不走 routeIntent：说话被当作造角色答复', async () => {
  ...
  try {
    const session = newVoiceSession();
    await ws(store, session, { type: 'voice_transcript', worldId: 'default', characterId: fairyId, transcript: '我想要一只小猫' });
    await answerRecipient(store, session, fairyId); // A2：先过 recipient 步，才进属性追问
    assert.equal(session.creation?.attrs.kind, '猫');
