...
| `list_flows` | 列 Flow Registry 注册的 flow：`{name,desc,kind,tags,args_schema,depends,requires,provides}` + `available:{ok,reasons}`（按当前 state 算,对齐 action 的 enabled/reason_disabled） |
| `run_flow` | 按名跑注册流程（经 `pilot_cli.py run-flow` 子进程,先跑 `depends` 链,本体前按 `requires` 硬校验）：回 `{ran,coverage,delta,duration}` |

`list_flows`/`run_flow` 经 `pilot_cli.py list-flows`/`run-flow` 子进程跑（`src/flow_runner.ts` 桥），与 CLI/web 同一执行路径；
流程定义在 `test/e2e/flows/`（`registry.json` + `<name>.py`）。`requires`/`provides` 的条件键（`in_world`/`online`/
`villagers_ready`/`vc_ready`）在 `flows/registry.py` 的 `CONDITIONS`；细节见 game-pilot SKILL.md「前置复用」节。
