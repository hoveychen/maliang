...
- **Python CLI（脚本化 / 回归 / MCP 没注册时）**：`python3 test/e2e/pilot_cli.py [--port P] [--trace DIR] <子命令>`，回 JSON。**命令面与 MCP 对等（同名子命令）**，flow 引擎也在同一 CLI（`list-flows`/`run-flow`），外加 `save-as-flow`（把录制的 trace 序列化成声明式 flow）。用于脚本化回归、以及 MCP 没注册时。
...
- **teleport（瞬移）已整个退役**——导航一律 `do walk:…`/`do enter_portal:…` 真走。

legacy 命令（`tap`/`click`/`talk-npc`/`phone`/`pick`/`scene`/`teleport`…）**已整个退役**——命令面与 MCP 对等。
capability 全经 `actions`→`do`（把"看到→走过去→点中"走全，暴露旧后门跳过的盲区：命中测试、遮挡、遮罩吞点击）；
盲坐标/设备手势**没有逃生口**了——某手势若 `actions` 里没有，是它还没做成 action_id（该补，不是绕）。
...
1. `test/e2e/flows/<name>.py` 暴露 `def run(h, **args): ...`（抛 `HarnessError`/`AssertionError` = 失败）。
   **★ flow 收到的 `h` 是 `MonkeyHarness`（玩家 SDK，不是 god 脚本）——诚实是类结构层面的**：这个类**根本不含**
   ...
   （`MonkeyHarness` 现在是**唯一** SDK 类——legacy/god 子类已删，flow 无从调盲坐标/瞬移。范例见
   `flows/enter_world.py`、`flows/naming_e2e.py` 的 `run()`。）`setup` 型要**幂等**。
2. 在 `test/e2e/flows/registry.json` 登记一条：`{name, desc, kind, tags, args_schema, depends}` + **`script` 或 `steps` 二选一**（`script`=命令式 `.py` 带 `run(h)`；`steps`=声明式数据步骤，见 §3 录制→存 flow）。
...
2. **一切交互经 `actions`→`do <id>`**：CLI 已砍成与 MCP 对等，legacy 盲坐标/专用子命令（`tap`/`click`/`talk-npc`/`phone`/`pick`/`scene`/`teleport`…）**都没了**。capability 全经 `do <action_id>`：进对话 `do talk:npc:<id>`/`do talk:player`、点按钮 `do press:btn:<path>`（发真触屏、会被遮罩正确吞掉，暴露"真孩子点不点得动"）、造物点卡 `do press:btn:<卡path>`、采纳 `do confirm:confirm_accept`、进场 `do enter_portal:<scene@x,y>`、走到 POI `do walk:poi:<id>`。**盲坐标/瞬移已整个退役**——没有绕过 `do` 的路；某动作 `actions` 里没有 = 它还没做成 action_id（该补而不是绕）。
...
**手机 / 场景切换 / 走位**：手机 `do phone:open`，屏内元素 `access` 枚举（viewport=PhoneScreen）后 `do press:btn:<path>`；跨场景 `do enter_portal:<scene@x,y>` 后 `wait --key transitioning --falsy`；走到 POI `do walk:poi:<id>`。**任意点盲走 / 按住跟随 / 滑动翻页**：走位用 `do walk:…`/`do walk:poi:…` 真走；没有对应 `do` action 的手势不再有盲坐标逃生口——需要就把它做成 action_id。

**整链回归（不要手搓重写）**：优先跑现成 flow（`list-flows` 看有哪些，`run-flow <name>` 跑，含造物起名 `naming_e2e`）。要新链路就**录一段再存成 flow**（见下），别写一次性脚本。

**录制→存成 flow（摸索出交互→存下来跑回归）**：驱动时带 `--trace <dir>`，把 `do/say/wait/inject` 命令流录进 `trace.jsonl`；然后 `pilot_cli.py save-as-flow <dir> --name <flowname> [--depends enter_world]` 序列化成**声明式 flow**（数据步骤，注册进 registry.json），之后 `run-flow <flowname>` 复跑。trace 里混进任何 legacy 盲坐标动作会被**当场拒**——声明式 flow 结构上只装得下定义词汇。
带逻辑（循环/条件/断言）的链路仍写命令式 `flows/*.py`（暴露 `def run(h): ...`，只用 `MonkeyHarness` 定义词汇 `h.do()/h.say()/h.access()/h.actions()/h.wait_*()`），登记时用 `script` 字段。范式见 `flows/naming_e2e.py`。

## 4. 轨迹与报告

长任务/巡检一律带 `--trace <dir>`：每条命令+应答追加 `trace.jsonl`、截图落同目录，事后可回放审计。
**回放回归**：把录下的 `trace.jsonl` 用 `save-as-flow` 存成声明式 flow，再 `run-flow <name>` 复跑——"上次这么点一遍是通的，这次还通吗"由 flow 承载（trace 不再单独回放）。
