...
用法：gen_pattern_tex.py <kind> <out.png> [--size 1024] [其它 kind 专属参数]
  kind ∈ crosswalk / carpet / puzzle / checker / vinyl / glow / hazard
       / wall_toy / wall_kitchen / wall_hospital / wall_future（室内房间墙面）
"""
import sys, argparse
import numpy as np
...
def main():
    ...
    elif a.kind == "wall_kitchen":
        # 厨房白瓷砖墙：瓷砖白底 + 浅灰勾缝网格（细缝），无缝可平铺、低对比契合卡通光滑画风。
        base = hx(a.base or "eef2f3"); fg = hx(a.fg or "c6ccd0")
        out[:] = base
        cell = n // a.period
        grout = ((xx % cell < 5) | (yy % cell < 5))
        out[grout] = fg
        # 每格瓷砖中心极轻高光（釉面感）
        px = (xx % cell) - cell / 2.0
        pz = (yy % cell) - cell / 2.0
        gloss = np.clip(1.0 - (px * px + pz * pz) / (cell * cell * 0.5), 0, 1)
        out = np.clip(out + gloss[..., None] * 0.02, 0, 1)
        out += paper_grain(rng, n, 0.01)
        out = soften(out, 1.2)

    elif a.kind == "wall_hospital":
        # 医院浅色墙：极浅薄荷 plaster 近纯色 + 极轻柔低频斑驳（灰泥质感），无图案、干净安静。
        base = hx(a.base or "e2ede8")
        out[:] = base
        mott = soften(rng.normal(0, 1, (n, n, 1)).repeat(3, 2), 8.0)
        out = np.clip(out + mott * 0.025, 0, 1)
        out += paper_grain(rng, n, 0.01)
        out = soften(out, 1.0)

    elif a.kind == "wall_future":
        # 未来金属舱壁：冷银底 + 面板分块线（较深银缝）+ 横向轻拉丝，科幻舱壁感、无缝可平铺。
        base = hx(a.base or "aab2ba"); fg = hx(a.fg or "6d7883")
        out[:] = base
        cell = n // a.period
        panel = ((xx % cell < 6) | (yy % cell < 6))
        out[panel] = fg
        # 横向拉丝（低频正弦条纹）
        brush = np.sin(yy * (2.0 * np.pi / 8.0)) * 0.012
        out = np.clip(out + brush[..., None], 0, 1)
        out += paper_grain(rng, n, 0.012)
        out = soften(out, 1.0)
