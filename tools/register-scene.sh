...
#     tools/register-scene.sh <world_id> <scene_id> <name> <mltr_path> [pois_json_path] [portals_json_path] [homes_json_path]
#
# pois/portals/homes 三个 json 由导出脚本一并产出（同名 .pois.json / .portals.json / .homes.json）。村庄与森林的
# 传送点互指，两张图都必须带 portals 重新入库，否则只有单向出口。homes 是「谁的家」建筑住户表
# （interaction-feedback B 档）：点村舍时点点说「这是别人的家呀」，缺省 [] 则一律走通用布景解释。
...
HOMES_JSON="[]";   [ -n "$HOMES" ]   && [ -f "$HOMES" ]   && HOMES_JSON=$(cat "$HOMES")

BODY=$(jq -n --arg w "$WORLD" --arg s "$SCENE" --arg n "$NAME" --arg b "$B64" \
  --argjson p "$POIS_JSON" --argjson q "$PORTALS_JSON" --argjson h "$HOMES_JSON" \
  '{worldId:$w, sceneId:$s, name:$n, terrainBase64:$b, pois:$p, portals:$q, homes:$h}')
