...
var saw_camera_pinned := false   ## 注魔测档期镜头被钉住（focus_override 非 INF）
var camera_stable := true        ## 注魔测档期镜头【不动】——各 trial 取景一致才可比；变了即噪声源，置 false
var _bench_focus                 ## 注魔期首见的 focus_override（Variant，null=未见）
...
func _tick() -> void:
	...
	if bench_running:
		saw_bench_node = true
		# 注魔测档期镜头必须【钉住且不动】：非 INF = 钉住；若期间变化 = 取景抖(噪声源) → camera_stable 置 false
		var fo: Vector2 = scene.get("focus_override")
		if fo != Vector2.INF:
			saw_camera_pinned = true
			if _bench_focus == null:
				_bench_focus = fo
			elif (_bench_focus as Vector2).distance_to(fo) > 0.01:
				camera_stable = false
...
func _finish() -> void:
	...
	_check("注魔测档期镜头钉住（focus_override 非 INF）", saw_camera_pinned, true)
	_check("注魔测档期镜头不动（各 trial 取景一致可比，杀噪声）", camera_stable, true)
	# —— 定档收尾：就地应用 + 不换场景 ——
