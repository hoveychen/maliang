...
func _init() -> void:
	...
	for interior_id in EX.VILLAGER_PORTALS:
		...
		if not found.is_empty():
			...
			# 村民室内现 9×9，落点 = 后排中线 (23,22)（room_back_landing 派生）
			fails += _check("%s 进门落室内后排中线" % interior_id, found["to_tile"], World.room_back_landing(interior_id))
	...
	for interior_id in COMPOSE.VILLAGER_INTERIOR_IDS:
		...
		# 房间 9×9 [19..27]²（room_n_for 派生）：家具须全落其中。
		var rn := World.room_n_for(interior_id)
		var lo := 19
		var hi := 19 + rn - 1
		var non_floor := 0
		var furn := 0
		...
		fails += _check("%s 家具件数与清单一致" % interior_id, furn, COMPOSE.VILLAGER_HOME_FURNITURE.size())
		fails += _check("%s 家具全落 9×9 房间内" % interior_id, out_of_room, 0)

		# 返回门（前开口边缘中线 (23,27)）→ village_forest，落点 = 该房子 return
		...
		if not inside.is_empty():
			...
			fails += _check("%s 返回门 tile = 前开口中线" % interior_id, inside[0]["tile"], World.room_front_tile(interior_id))
			fails += _check("%s 返回落点对" % interior_id, inside[0]["to_tile"], Vector2i(ret[0], ret[1]))
			# 进屋落点（后排）离返回门 > radius（防弹回）
			var at_enter := WorldGrid.from_tile_center(World.room_back_landing(interior_id))
