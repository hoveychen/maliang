...
## 「帮我变一朵会点头的小花」→ 意图路由 create_prop → designSdfProp → prop_created
...
func _tick() -> void:
	...
	if say.is_empty():
		say = "帮我变一朵会点头的小花"
	scene.get("backend").prop_created.connect(func(p: Dictionary) -> void: print("[qa] prop_created ", String(p.get("id", ""))))
