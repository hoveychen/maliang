...
三种接入：
  桌面 debug 实例   MonkeyHarness()（直连 127.0.0.1:8577；多实例并存用 MALIANG_HARNESS_PORT 换口）
  Android 真机      setup_forward() 后 MonkeyHarness()（adb forward 把设备口映到本机）
  桌面自起实例      launch_desktop(port=8578) 起 Godot debug 进程再 MonkeyHarness(port=8578)
...
# ── legacy/debug 子类已删（harness-declarative-flows P4：词汇收口）──────────────
# 曾有 Harness(MonkeyHarness) 提供盲坐标手势(tap/drag/swipe/long_press/pinch)、直触 handler
# (pick/accept/click_ui/phone/pickup/talk_*)、god op(teleport/scene/photo/reset_budget)，
# 只服务已删的 `run-script` 逃生口。现在驱动面只剩 MonkeyHarness 的定义词汇——任何 flow
# （命令式或声明式）都调不到 legacy，「不泄漏非定义词汇的动作」是类结构层面的保证。
# 需要盲坐标/god op 的老脚本一并退役；导航走 do('walk:…')/do('enter_portal:…') 真走。
