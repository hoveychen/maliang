...
用法：
  ...
  python3 test/e2e/flows/naming_e2e.py --device --launch --intent "点点，帮我造一个火箭"
  # 已在世界里、只驱语音（不重启 App）：
  python3 test/e2e/flows/naming_e2e.py --device --intent "点点，帮我造一个火箭" --name "冲天号"
  # 桌面 debug（直连，不走 adb）：
  python3 test/e2e/flows/naming_e2e.py --host 127.0.0.1 --intent ... --name ...
...
sys.path.insert(0, str(Path(__file__).parent.parent))  # flows/ → test/e2e/（harness.py 在此）
