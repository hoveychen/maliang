...
## 编排（--fixed-fps 8）：1s 委托 chip 亮起 → 2s 委托完成（灵狐小围巾跳+🎉+盖章飞入手机按钮）→
		...
		8:
			scene.call("_on_world_state", { "wallet": { "flowers": 2, "stampProgress": 2, "stampsTotal": 5 },
				"activeTask": {
					...
					"npcName": "灵狐小围巾", "targetName": "舞舞兔", "message": "你好呀", "stampStyle": "star" } })
		16:
			scene.call("_on_task_complete", { "task": { "id": "t", "type": "deliver",
				"npcId": String(green.get("id", "")), "npcName": "灵狐小围巾", "stampStyle": "medal" },
