extends SceneTree
## 第二张地图森林（scene-portal P5；scene-items 后布置进矩阵物品层）：
##   1. export_forest 生成的 .mltr 合法、可载入、与村庄地形不同（changed=true）；含小河（水）+
##      高地空地（height>0）。
##   2. 场景感知在导出组装（scene_compose）：同一张森林地形上，forest 规则铺满树（树数量远多于
##      village 规则）——证明布置规则场景感知真的生效。
...
func _init() -> void:
	...
	# ── 布置规则场景感知：森林地形上 forest 规则铺满树，远多于 village 规则 ───────
	var forest_trees := _count_trees("forest", n)
	var village_trees := _count_trees("village", n)
...
func _count_trees(scene_id: String, n: int) -> int:
	var c := 0
	for z in range(n):
		for x in range(n):
			if COMPOSE._deco_kind(scene_id, Vector2i(x, z)) == COMPOSE.DECO_TREE:
				c += 1
	return c
