...
func _run_once() -> void:
	...
	print("[build_actions：世界外(menu)不出世界级动作 + 世界内按状态事实]")
	# ★menu/onboarding(in_world 缺省 false)：say/confirm/phone 一个都不该冒(老板在标题页看到 6 动作即此 bug)
	fails += _check("非世界(menu)build_actions 空", HarnessAccess.build_actions({"mic_open": true, "phone_open": false}).size(), 0)
	# 以下均在世界内(in_world:true)
	var g1 := HarnessAccess.build_actions({"in_world": true, "mic_open": true})
	fails += _check("开麦 say 可用", _find(g1, "say")["enabled"], true)
	var g2 := HarnessAccess.build_actions({"in_world": true, "mic_open": false})
	fails += _check("闭麦 say 禁用", _find(g2, "say")["enabled"], false)
	fails += _check("闭麦 say reason", _find(g2, "say")["reason_disabled"], "mic_closed")
	var g3 := HarnessAccess.build_actions({"in_world": true, "mic_open": true, "act_gated": true, "gate_reason": "blocked_by_overlay"})
	...
	# 统一门:loading/intro(_bench_freeze)、回家、换场景 等任一 gated 都禁交互并带 reason
	var g3b := HarnessAccess.build_actions({"in_world": true, "mic_open": true, "act_gated": true, "gate_reason": "loading_intro"})
	fails += _check("loading/intro 下 say 禁用", _find(g3b, "say")["enabled"], false)
	fails += _check("loading/intro reason 透传", _find(g3b, "say")["reason_disabled"], "loading_intro")
	var g4 := HarnessAccess.build_actions({"in_world": true, "vc_confirming": true})
	fails += _check("确认态 采纳可用", _find(g4, "confirm:confirm_accept")["enabled"], true)
	var g5 := HarnessAccess.build_actions({"in_world": true, "vc_confirming": false})
	fails += _check("非确认态 采纳禁用", _find(g5, "confirm:confirm_accept")["enabled"], false)
	var g6 := HarnessAccess.build_actions({"in_world": true, "in_creation": true, "creation_options": [{"id": "opt_a", "label": "A"}]})
	fails += _check("造物态【不再】造 pick_option 业务后门(改走通用 press:btn 真 tap)", _find(g6, "pick_option:opt_a").is_empty(), true)
	var g7 := HarnessAccess.build_actions({"in_world": true, "phone_open": false})
	fails += _check("手机关→有打开动作", _find(g7, "phone:open").is_empty(), false)
	var g8 := HarnessAccess.build_actions({"in_world": true, "phone_open": true})
