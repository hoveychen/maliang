...
const LAYER_YELLOW_BRICK := 48  ## 黄砖路（绿野仙踪；复用 paver_brick 贴图，mean 归一化 + 金黄 tint 着成金砖）
const LAYER_COUNT := 49
...
const LAYER_TEX_PATHS: Array[String] = [
	...
	"res://assets/textures/terrain/paver_brick.png",   # 48 YELLOW_BRICK（复用砖纹，色由 tint/mean 归一成金砖）
]
...
static func layer_tints() -> PackedColorArray:
	...
	# 黄砖路：mean 归一化掉 paver_brick 的砖红底后，tint 直接定「干净金砖」色（配合 layer_means 的实测均值）。
	t[LAYER_YELLOW_BRICK] = Color(0.95, 0.70, 0.20)
	return t
...
static func layer_means() -> PackedColorArray:
	...
	# 黄砖路 mean = paver_brick.png 实测 sRGB 均值（magick 量得 105/89/81），tex/mean 归一后色由 tint 定。
	m[LAYER_YELLOW_BRICK] = Color(0.412, 0.350, 0.318)
	return m
...
static func top_layer(ttype: int) -> int:
	match ttype:
		...
		TerrainMap.T_YELLOW_BRICK: return LAYER_YELLOW_BRICK
	return LAYER_GRASS
