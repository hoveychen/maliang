...
var _last_pcm := PackedByteArray()  ## 最近一次【采纳】的整段录音 PCM（B3 起名：宿主取来打包上传当语音名）
var _player: AudioStreamPlayer = null ## 录音回放（模块自持：宿主不必知道回放这回事）

## 当前活跃的 VoiceCapture 实例：app 级 harness 命令口跨场景据此定位它——onboarding 页与 world 各有一个
## VC，谁在树上 current 就指谁，取代原来「DebugCmdServer 绑死 world._vc」（见 debug_cmd_server.gd）。
static var current: VoiceCapture = null
...
func _exit_tree() -> void:
	...
	if current == self:
		current = null  # 让出「当前活跃 VC」——harness 命令口据此定位（见 debug_cmd_server.gd）
	close()
	_disconnect_asr()
...
## 最近一次【采纳】的整段录音原始 PCM（16k/16bit/mono）。B3 起名：宿主在 local_final 里取来上传。
## 只在确认模式下攒得到（_flush 仅确认模式累积 _utt_pcm）；非确认模式恒空。
func last_pcm() -> PackedByteArray:
	return _last_pcm
...
func accept() -> void:
	...
	_last_pcm = _utt_pcm.duplicate() # 采纳的这段留一份原始 PCM 供宿主取（_end_confirm 会清 _utt_pcm）
	_end_confirm()
	if game_audio != null:
