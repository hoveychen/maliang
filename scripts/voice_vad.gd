...
## 触发阈值下限（归一化 RMS）。真机实测：安静房间噪声底仅 0.002–0.007，noise×MARGIN 恒被
## 钳到此下限；而真人说话 rms≈0.06–0.08。旧值 0.015 太低，呼吸/桌面轻碰/BGM 回升尾音都能
## 触发开口，录到近静音 → 端侧 ASR 返回空 → 立刻重开麦 → 连环录。抬到 0.030 仍给真人声留
## 约 2× 余量；噪声环境下自适应阈值本就高于下限，不受影响。数值可随 [vad] logcat 继续迭代。
const ABS_MIN := 0.030
