...
##   row 5..8     水（R/B/A = 湖床主体，供地面 V_FULL cell 用；G = 水面泡沫带——沿旧水岸
##                autotile 几何烘的圆角边带，水面 mesh 按角变体采样，窄溪也只在贴岸处起沫）
...
## 17 基础行（草/路/水/崖唇/崖壁）+ 每种新地表 4 行（NW/NE/SW/SE 角变体），world-themes P1 加 3 种。
const ROWS := 17 + 4 * 3

## 新增可行走地表的行基址（各占 4 行角变体，克隆路的过渡几何，仅换 B 通道 body-type）。
## 键 = TerrainMap.T_*，值 = 起始行；与 shader body-type ty=round(B*8) 5/6/7 一一对应。
const BODY_ROWS := { TerrainMap.T_SAND: 17, TerrainMap.T_SNOW: 21, TerrainMap.T_TILE: 25 }
const W := COLS * PITCH   ## 200
const H := ROWS * PITCH   ## 680

## uv_rect 的扩展"类型"码（排在 TerrainMap 三个真实类型之后）。
const CLIFF_RIM := 3   ## 悬崖边草皮（地表）
const CLIFF_WALL := 4  ## 崖壁（竖直墙面）
...
## 新增地表 tint（world-themes P1；复用 dirt/stone 细节贴图，靠 tint 出质感差异）
const SAND_TINT := Color(0.90, 0.80, 0.55)   ## 沙地暖黄（比路更饱和、无描边）
const SNOW_TINT := Color(0.93, 0.95, 0.99)   ## 雪地冷白（略偏蓝，压住 stone 细节）
const TILE_TINT := Color(0.72, 0.74, 0.78)   ## 瓷砖中性冷灰（室内地板）
...
static func uv_rect(type: int, corner: int, variant: int, parity: int) -> Rect2:
	...
	if type == TerrainMap.T_GRASS:
		...
		col = variant
		row = 9 + corner
	elif BODY_ROWS.has(type):
		col = variant
		row = int(BODY_ROWS[type]) + corner
	else:
		col = variant
		row = 13 + corner
	return Rect2(
...
static func build_image() -> Image:
	...
	for corner in range(4):
		for variant in range(Autotile.VARIANT_COUNT):
			...
			# 新增地表：克隆路的过渡几何，仅换 B 通道 body-type（= TerrainMap.T_*，对齐 shader ty 5/6/7），无描边
			for body_ty in BODY_ROWS:
				_fill_cell(img, variant, int(BODY_ROWS[body_ty]) + corner, _body_fn(corner, variant, body_ty))
	return img
...
## 新增可行走地表 cell（沙/雪/瓷砖）：几何同路过渡（草底上圆角 autotile），
## 但 body B = 该地表类型（shader 按 ty 选 tint/细节贴图），且无描边（G=0）——
## 沙滩/雪原/地板与草的边界要柔和过渡，不像路那样有亮边。整格 V_FULL cell → R=1 铺满。
static func _body_fn(corner: int, variant: int, body_ty: int) -> Callable:
	return func(x: float, y: float) -> Color:
		var cx := x if (corner == Autotile.C_NW or corner == Autotile.C_SW) else CELL - x
		var cy := y if (corner == Autotile.C_NW or corner == Autotile.C_NE) else CELL - y
		var d := _signed_dist(cx, cy, variant)
		var b := float(body_ty) / 8.0
		if d < 0.0:
			return _grass_ctl(x, y, LUM_GRASS_MID, b)  # 草底（带 body B，线性过滤下类型恒定）
...
static func _water_fn(corner: int, variant: int) -> Callable:
	return func(x: float, y: float) -> Color:
		...
		return Color(1.0, foam, float(TerrainMap.T_WATER) / 8.0, _speckle_lum(x, y) * 0.5)
