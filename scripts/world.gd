...
const VILLAGER_BASE_HEIGHT := 3.0 * WorldGrid.TILE_SIZE ## 村民/角色世界高度基准=3 tile×TILE_SIZE(=6.0，中号体型)；×appearance.scale 得实际高度
...
## 室外环境默认（_setup_environment 与切回室外时共用的单一来源；改这里就一起变，不再散字面量）。
const OUTDOOR_AMBIENT_COLOR := Color(0.62, 0.70, 0.92) ## 冷调环境光（Pokopia 式色相分离）
...
## 网格必须 25 整除故室内一律 50，房间只占其中 N×N 一小块，其余地形隐藏。房间几何【按场景】
...
## 玩家按相对房间中心的最短位移在屋里走），固定俯角 + 【按房间尺寸算出的、刚好框满全屋的最近距离】。
## 俯角比室外 god(47°) 略陡但不再像原来那样贴顶俯视（62→50，进屋更像「探进屋里看」而非平面图）。
## 距离不再写死：_indoor_cam_dist_for(n) 按 N/俯角/竖直 FOV 二分求「全屋恰好入镜的最近轨道距离」。
const INDOOR_CAM_PITCH := 50.0    ## 室内俯角（度）——放平进屋、看得见后墙立面与家具站立感；比室外 god 略陡
const INDOOR_FIT_MARGIN := 1.06   ## 框满全屋后再退开一点余量（1.0=严丝合缝，留一点边不贴框）
const CAM_FOV_DEG := 50.0         ## 相机竖直 FOV（KEEP_HEIGHT）；单一来源，_setup_camera 与室内框图反算共用
...
const INDOOR_SUN_COLOR := Color(1.0, 0.86, 0.66)      ## 更暖
const INDOOR_SUN_ENERGY := 0.55                        ## 压暗（窗光/灯光柔光感）
...
## 角色动画 LOD（CharAnimLod，泛化上面的 video-hero focus）：按到玩家的距离连续选最近 N 个
## 角色升「高保真档」（24fps 图集）。视频受解码限 ≤2 路，图集无解码开销故 N 可更大。
## enter → 拉 hiAnimAsset 切 24fps；leave → 切回底座 8fps animAsset（见 _step_char_lod）。
var _char_lod := CharAnimLod.new()
var _char_lod_t := 0.0            ## LOD 更新节流累计（接入 _process 热路径，~0.4s 一次）
const CHAR_LOD_INTERVAL := 0.4
## 立绘 hash → 两档图集信息 { lo, lo_meta, hi, hi_meta }（服务端 /sprite-anim 下发，按 sprite_hash 去重）。
## LOD 切档时按此拉图集，免得每次 enter/leave 都打一次 fetch_sprite_anim 网络请求。
## 在 _prefetch_one / _poll_idle_anim 拿到 rec 时填（见 _remember_char_anim）。
var _char_anim := {}
...
func _setup_environment() -> void:
	...
	light.light_color = OUTDOOR_SUN_COLOR # 暖阳（比旧值更暖：与冷环境光拉开色相分离）
	light.light_energy = OUTDOOR_SUN_ENERGY
	...
	env.ambient_light_color = OUTDOOR_AMBIENT_COLOR
	env.ambient_light_energy = OUTDOOR_AMBIENT_ENERGY
...
func _step_sky(delta: float) -> void:
	if _env == null or _env.background_mode != Environment.BG_SKY:
		return # 室内场景切了 BG_COLOR、无天空可漂移（_env.sky 仍非空，但不渲染）
	_sky_step_t += delta
...
## 按当前场景切换室内/室外环境观感（home-interior P3）。进出 home_interior 时调用：
...
func _apply_scene_env(scene_id: String) -> void:
	...
	if _env != null:
		if indoor:
			...
			_env.background_color = INDOOR_BG_COLOR
			_env.fog_enabled = false
			_env.ambient_light_color = INDOOR_AMBIENT_COLOR
			_env.ambient_light_energy = INDOOR_AMBIENT_ENERGY
		else:
			_env.background_mode = Environment.BG_SKY
			_env.fog_enabled = int(_gfx_levels.get("fog", 1)) > 0
			_env.ambient_light_color = OUTDOOR_AMBIENT_COLOR
			_env.ambient_light_energy = OUTDOOR_AMBIENT_ENERGY
	if _sun != null:
...
func _apply_indoor_render(scene_id: String) -> void:
	var indoor := _scene_is_indoor(scene_id)
	var n := room_n_for(scene_id)   # 房间边长按场景（普通人家 9 / 七矮人·城堡 12），出室外为 0
	if chunk_manager != null:
		chunk_manager.set_terrain_hidden(indoor)
		# 壁挂物：室内把房间周界告诉 chunk_manager，周界墙边贴纸抬墙高（home-wall-decor P1）。
		chunk_manager.set_room_bounds(ROOM_ORIGIN_TILE, n if indoor else 0)
	if room_stage != null:
		if indoor:
			room_stage.build(n)
		else:
			room_stage.clear()
	if indoor:
		# 相机收束到整屋：focus 钉房间中心，俯角放平（INDOOR_CAM_PITCH），距离按房间尺寸算「刚好框满全屋
		# 的最近轨道距离」（_indoor_cam_dist_for，非写死——9×9 自然比 12×12 拉得近）。进屋即把 _cur_* 直接
		# 对齐目标（不缓动），揭幕那一刻就是放平的室内视角，不会先闪一下外面的俯视再滑进来。
		_indoor_cam_dist = _indoor_cam_dist_for(n)
		focus_logical = _room_center_logical()
		_target_pitch = INDOOR_CAM_PITCH
		_target_dist = _indoor_cam_dist
		_cur_pitch = INDOOR_CAM_PITCH
		_cur_dist = _indoor_cam_dist
		_cur_focus_y = 0.0   # 室内地板全平，焦点高度恒 0
	else:
...
## 不必改这里）。房间边长按场景（room_n_for），几何左上角共用 ROOM_ORIGIN_TILE。
func _scene_is_indoor(sid: String) -> bool:
	return sid.ends_with("_interior")

## 房间边长（tile 数）按场景：普通人家/农舍/外婆家 9×9，七矮人小屋/翡翠城堡 12×12；非室内为 0。
## 静态：export_terrain.gd（传送门/落点）与本文件共用同一套房间几何——单一真相，别在别处再抄。
static func room_n_for(sid: String) -> int:
	if not sid.ends_with("_interior"):
		return 0
	return ROOM_N_LARGE if sid in LARGE_INTERIOR_IDS else ROOM_N_COZY

## 房间中线所在列（tile）：origin.x + (N-1)/2。N=12→24（与旧几何一致，snow/城堡门不动）；N=9→23。
static func room_center_col(sid: String) -> int:
	return ROOM_ORIGIN_TILE.x + (room_n_for(sid) - 1) / 2

## 室内返回门 tile：前开口边缘（最大 y = origin.y+N-1）的中线列。走向前开口 = 走出门。
static func room_front_tile(sid: String) -> Vector2i:
	return Vector2i(room_center_col(sid), ROOM_ORIGIN_TILE.y + room_n_for(sid) - 1)

## 进屋落点 tile：后排中线（origin.y+ROOM_BACK_INSET），离返回门远防落地即弹回。
static func room_back_landing(sid: String) -> Vector2i:
	return Vector2i(room_center_col(sid), ROOM_ORIGIN_TILE.y + ROOM_BACK_INSET)

## 当前场景的房间边长（tile），非室内为 0。
func _room_n() -> int:
	return room_n_for(_scene_id)

## 房间中心的逻辑坐标（世界单位）：区间 [origin .. origin+N-1] 的几何中心 = (origin + N/2)×TILE。
func _room_center_logical() -> Vector2:
	var n := float(_room_n())
	var c := (Vector2(ROOM_ORIGIN_TILE) + Vector2(n, n) * 0.5) * WorldGrid.TILE_SIZE
...
func _clamp_tile_to_room(t: Vector2i) -> Vector2i:
	var n := _room_n()
	return Vector2i(
		clampi(t.x, ROOM_ORIGIN_TILE.x, ROOM_ORIGIN_TILE.x + n - 1),
...
func _poll_idle_anim(node: PaperCharacter, sprite_hash: String, world_height: float, phase: float) -> void:
	...
	while true:
		...
		if status == "ready":
			_remember_char_anim(sprite_hash, rec)  # 记两档图集信息供 LOD 切档
...
func _process(delta: float) -> void:
	...
	_step_char_lod(delta)
	tp = _prof_lap(tp, "charlod")
	...
	if focus_override != Vector2.INF:
		...
		# 高优先分支仍照常覆盖（进屋跟村民聊天照样拉近构图）。距离用进屋算好的 _indoor_cam_dist
		# （按房间尺寸框满全屋）；万一未初始化（0）就现算一次兜底。
		want = _room_center_logical()
		_target_pitch = INDOOR_CAM_PITCH
		if _indoor_cam_dist <= 0.0:
			_indoor_cam_dist = _indoor_cam_dist_for(_room_n())
		_target_dist = _indoor_cam_dist
...
## 角色动画 LOD 步进（节流 ~0.4s；接入 _process 热路径）：按到玩家的环面距离选最近 N 个角色为
## 高保真集，enter/leave 由 CharAnimLod 决定。enter → 拉该角色 24fps 高保真图集切 play_anim +
...
func _step_char_lod(delta: float) -> void:
	if not online or player.is_empty() or npcs.is_empty():
		return
	_char_lod_t += delta
	if _char_lod_t < CHAR_LOD_INTERVAL:
		return
	_char_lod_t = 0.0
	var player_l: Vector2 = player["logical"]
	var chars: Array = []
	...
	for n in npcs:
		if bool(n.get("is_fairy", false)):
			continue  # 仙子不走 LOD（她另有固定档，且不参与"最近 N"）
		var node: Variant = n.get("node", null)
		if node == null or not is_instance_valid(node):
			continue
		# 对话焦点角色已升「视频档」(24fps 真视频,_begin_video_heroes)——比图集档更高保真,
		# 不纳入图集 LOD,免得这里 play_anim 把真视频盖回图集。退出对话(stop_video_lod)后自动重入。
		if node.is_video_lod():
			continue
		var id := String(n.get("id", ""))
		if id.is_empty():
			continue
		chars.append({ "id": id, "dist": WorldGrid.shortest_delta(n["logical"] as Vector2, player_l).length() })
		nodes[id] = node
		phases[id] = float(n.get("anim_phase", 0.0))
	var r := _char_lod.update(chars)
	for id in r["enter"]:
		var node: PaperCharacter = nodes.get(id, null)
		if node != null:
			_raise_char_hi(id, node, float(phases.get(id, 0.0)))  # fire-and-forget 拉 24fps 切档
	for id in r["leave"]:
		var node: PaperCharacter = nodes.get(id, null)
		if node != null:
			_lower_char_lo(node, float(phases.get(id, 0.0)))  # 切回底座 8fps
	if OS.is_debug_build() and (not (r["enter"] as Array).is_empty() or not (r["leave"] as Array).is_empty()):
		print("LOD hi=%d enter=%s leave=%s" % [_char_lod.hi_count(), str(r["enter"]), str(r["leave"])])

## LOD 升档（fire-and-forget）：拉该角色 24fps 高保真图集，切到 play_anim 并登记进 _char_lod 强引用池。
## 无 hi 变体（存量未回填 / 生成中）→ 留在底座 8fps。fetch 期间角色可能已离集 → late 纹理不登记。
func _raise_char_hi(id: String, node: PaperCharacter, phase: float) -> void:
	if not is_instance_valid(node):
		return
	var info: Dictionary = _char_anim.get(node.sprite_hash, {})
	...
	if not _char_lod.is_hi(id) or node.is_video_lod():
		return  # fetch 期间已跌出最近 N / 已升视频档：late 纹理不登记（别盖真视频），交给局部引用释放
	node.play_anim(tex, info.get("hi_meta", {}), node.visible_height(), phase)
	_char_lod.hold(id, tex)

## LOD 降档（fire-and-forget）：切回底座 8fps 图集。_char_lod 已在 update() 丢掉 hi 引用，
## 这里把节点纹理也换回 lo（否则节点仍持 hi 纹理）。无 lo 记录（异常）→ 保持现状不动。
func _lower_char_lo(node: PaperCharacter, phase: float) -> void:
	if not is_instance_valid(node):
		return
	var info: Dictionary = _char_anim.get(node.sprite_hash, {})
	var lo_hash := String(info.get("lo", ""))
	if lo_hash.is_empty():
		return
	var tex := await api.fetch_texture(lo_hash, true)
	if tex == null or not is_instance_valid(node) or node.is_video_lod():
		return  # 升视频档期间别把真视频盖回图集
	node.play_anim(tex, info.get("lo_meta", {}), node.visible_height(), phase)

## 记下某立绘的两档图集信息（LOD 切档时免网络查）。rec 为 fetch_sprite_anim 返回；非 ready / 无底座档 → 跳过。
func _remember_char_anim(sprite_hash: String, rec: Dictionary) -> void:
	if sprite_hash.is_empty() or String(rec.get("status", "")) != "ready":
		return
	var lo := String(rec.get("animAsset", ""))
	if lo.is_empty():
		return
	_char_anim[sprite_hash] = {
		"lo": lo, "lo_meta": rec.get("meta", {}),
		"hi": String(rec.get("hiAnimAsset", "")), "hi_meta": rec.get("hiMeta", {}),
	}
...
func _unload_scene() -> void:
	...
	for n in npcs:
		...
		if node != null and is_instance_valid(node):
			(node as Node).queue_free()
	npcs.clear()
	_char_lod.clear()  # 收 LOD 池:换场景角色全清,别持旧场景纹理引用
	_villager_count = 0
...
func _bootstrap_apply(fetched: Dictionary) -> void:
	...
	npcs.clear()
	_char_lod.clear()  # 收 LOD 池:离线占位角色清空,真村民降生后重新按距离入集
...
func _prefetch_one(c: Dictionary, results: Dictionary, pending: Array) -> void:
	...
	if not sprite.is_empty():
		...
		_remember_char_anim(sprite, rec)  # 记两档图集信息供 LOD 切档（近处角色升 24fps）
...
func _spawn_server_character(c: Dictionary, at_logical: Vector2, prefetched := {}) -> void:
	...
	var cid := _char_id(c)
	# 相位按 id 错开，避免整村同帧起跳的机械感（31帧/8fps 循环约 3.9s）。存进 npc 字典供 LOD 切档复用（观感不跳）。
	...
	var dict := { "node": npc, "logical": logical, "id": cid, "is_fairy": is_fairy, "scale": body_scale, "anim_phase": anim_phase }
...
func _snap_to_room_wall(want: Vector2i) -> Vector3i:
	...
	var maxx := ROOM_ORIGIN_TILE.x + n - 1
	var maxy := ROOM_ORIGIN_TILE.y + n - 1
...
func _refresh_place_ghost() -> void:
	...
	if _place_is_edge:
		...
		var is_wall := _is_indoor() and ChunkManager.is_room_wall_edge(_place_tile, _place_edge, ROOM_ORIGIN_TILE, _room_n())
		if is_wall:
