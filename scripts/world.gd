...
## 角色升「高保真档」（将来的 24fps 图集）。视频受解码限 ≤2 路，图集无解码开销故 N 可更大。
## 骨架期（server 24fps 素材未就位）：只算集合 + 持有占位纹理引用 + debug 日志，不切真图集。
...
## 高保真集，enter/leave 由 CharAnimLod 决定。骨架期：enter 的角色持有其**当前**（占位）纹理引用，
## debug 打印当前集——真 24fps 图集就位后，这里改成拉高帧图集 + play_anim 切档（离集则切回 8fps）。
func _step_char_lod(delta: float) -> void:
	...
	for id in r["enter"]:
		...
		if node != null and node.texture != null:
			_char_lod.hold(id, node.texture)  # 骨架占位；真 24fps 就位后换成拉高帧图集
