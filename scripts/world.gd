...
func _setup_hud() -> void:
	...
	task_chip.offset_left = -460.0
	task_chip.offset_top = 12.0
	task_chip.offset_right = -16.0
	task_chip.horizontal_alignment = HORIZONTAL_ALIGNMENT_RIGHT
	_style_label(task_chip, 26)
...
func _sticker_glyph(id: String) -> String:
	return STICKER_GLYPHS.get(id, "⭐")

## 委托提示 chip：🎯目标（emoji 为主，家长可读短名） ⇒ 奖励贴纸。
func _update_task_chip() -> void:
	...
	match String(active_task.get("type", "")):
		"deliver":
			goal = "💬→%s" % active_task.get("targetName", "")
		"bring":
			goal = "🤝%s→%s" % [active_task.get("targetName", ""), active_task.get("npcName", "")]
		"visit":
			goal = "📍%s" % active_task.get("locationName", "")
		"gift":
			goal = "%s→%s" % [_sticker_glyph(String(active_task.get("itemId", ""))), active_task.get("npcName", "")]
...
func _on_task_complete(data: Dictionary) -> void:
	...
	banner.visible = true
	_celebrate_reward(glyph, data.get("task", {}))
