...
var _hearts_label: Label            ## 收到的爱心计数（玩家互动送❤）
...
func _build_flowers_page() -> Control:
	...
	hearts_row.add_child(UiAssets.icon_rect("em_heart", 26.0))
	_hearts_label = Label.new()
	_style_card_label(_hearts_label, 22)
	hearts_row.add_child(_hearts_label)
	page.add_child(hearts_row)
	return page
...
func _refresh_album() -> void:
	...
	if _hearts_label != null:
		_hearts_label.text = "x%d" % int(wallet.get("hearts", 0))
	_refresh_items_page()
