...
# 奖赏系统：进行中委托 + 贴纸背包（服务端权威，world_state/task_complete 同步）
...
var album_button: Button           ## 左下角收集册按钮（AIGC 摊开书图标）
...
## 手机皮肤 id → 资产 + 机身几何。aspect=机身宽高比；screen=壳自带屏区占机身的内缩比例
## （有壳图时屏幕透明、内容直接画在壳的屏区上，机身+装饰当边框露出来；换手机各带自己的几何）。
const PHONE_SKINS := {
	"fairy": {
		"launcher": "ic_phone_fairy", "shell": "phone_shell_fairy",
		"aspect": 298.0 / 512.0,
		"screen": { "l": 0.115, "t": 0.095, "r": 0.115, "b": 0.105 },
	},
}
...
## 收集册展示顺序（与 server/src/types.ts STICKERS 对齐；图标映射见 UiAssets.STICKER_ICONS）
...
func _setup_hud() -> void:
	...
	# 机身尺寸：有壳图按壳的宽高比（贴图不拉伸变形），占位机身用竖屏近似。
	var pw := 360.0
	var ph: float = pw / float(skin.get("aspect", 360.0 / 660.0)) if has_shell else 660.0
	album_panel.custom_minimum_size = Vector2(pw, ph)
	# bezel：有壳图→贴合壳自带屏区(按皮肤比例内缩)，让机身+蜻蜓翼当边框露出；占位机身→小边距。
	var ins: Dictionary = skin.get("screen", {})
	var bezel := MarginContainer.new()
	if has_shell:
		bezel.add_theme_constant_override("margin_left", int(pw * float(ins.get("l", 0.1))))
		bezel.add_theme_constant_override("margin_right", int(pw * float(ins.get("r", 0.1))))
		bezel.add_theme_constant_override("margin_top", int(ph * float(ins.get("t", 0.1))))
		bezel.add_theme_constant_override("margin_bottom", int(ph * float(ins.get("b", 0.1))))
	else:
	...
	app_grid.columns = 4
	app_grid.add_theme_constant_override("h_separation", 12)
	app_grid.add_theme_constant_override("v_separation", 12)
...
func _make_app_icon(app: Array) -> Control:
	...
	var btn := Button.new()
	btn.custom_minimum_size = Vector2(58.0, 58.0)
	btn.icon = tex
...
func _make_blank_slot() -> Control:
	...
	box.add_theme_constant_override("separation", 4)
	var tile := Panel.new()
	tile.custom_minimum_size = Vector2(58.0, 58.0)
...
func _update_task_chip() -> void:
	...
	match String(active_task.get("type", "")):
		...
		"gift":
			task_chip.add_child(_chip_icon(UiAssets.sticker_tex(String(active_task.get("itemId", "")))))
			task_chip.add_child(_chip_label("→%s" % active_task.get("npcName", "")))
	task_chip.add_child(_chip_label("⇒"))
	task_chip.add_child(_chip_icon(UiAssets.sticker_tex(String(active_task.get("rewardId", "")))))
...
func _on_task_complete(data: Dictionary) -> void:
	...
	_refresh_album()
	banner.text = "太棒啦！得到一张新贴纸"
	banner.visible = true
	_celebrate_reward(String(data.get("rewardId", "star")), data.get("task", {}))
...
func _celebrate_reward(reward_id: String, task: Dictionary) -> void:
	...
	if game_audio != null:
		game_audio.play_sfx("enter")
	_fly_reward_to_album(reward_id)
...
func _fly_reward_to_album(reward_id: String) -> void:
	var l := TextureRect.new()
	l.texture = UiAssets.sticker_tex(reward_id)
...
func _refresh_album() -> void:
	for id in _album_cells:
		...
		(cell["glyph"] as TextureRect).modulate = Color.WHITE if n > 0 else Color(0.28, 0.28, 0.34)
...
## 贴纸从玩家头顶飞到受赠者头顶，停一拍消失（AIGC 贴纸 Sprite3D）。
func _spawn_gift_fly(item_id: String, d: Dictionary) -> void:
	var l := UiAssets.bubble_sprite(String(UiAssets.STICKER_ICONS.get(item_id, "st_star")), 1.7)
	l.visible = true
	add_child(l)
