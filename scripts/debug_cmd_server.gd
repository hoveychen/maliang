...
var _sampler: FrameSampler = null  ## measure_p95 进行中的采样器（跨帧异步；采满在 _process 回 p95）
var _saved_max_fps := 0            ## measure 期临时 uncap fps（照 Benchmark 先例），采完还原
...
static func parse_command(line: String) -> Dictionary:
	...
	match op:
		...
		"measure_p95":
			...
			var out := {"ok": true, "op": "measure_p95"}
			if dict.has("warmup"):
				out["warmup"] = float(dict["warmup"])
			if dict.has("window"):
				out["window"] = float(dict["window"])
			return out
...
func _process(delta: float) -> void:
	if _server == null:
		return
	_pump_measure(delta) # measure_p95 采样进行中：喂真实帧时，采满即回 p95（跨帧异步 op）
...
func _handle_line(line: String) -> void:
	...
	if not bool(cmd.get("ok", false)):
		...
		return
	var result := _execute(cmd)
	if bool(result.get("__defer__", false)):
		return # 异步 op（measure_p95）：采满后在 _process 回，这里不回
	_reply(result)
...
## A/B 差异），跨帧在 _pump_measure 喂真实帧 delta；采满回 {p95_ms, samples, tex_diag}。
## 返回 __defer__ 让 _handle_line 先不回复（采满才回）。已有采样在跑则拒绝（一次一个）。
func _do_measure_p95(cmd: Dictionary) -> Dictionary:
	if _sampler != null:
		return {"ok": false, "op": "measure_p95", "error": "measure already running"}
	var warmup := float(cmd.get("warmup", FrameSampler.WARMUP))
	var window := float(cmd.get("window", FrameSampler.WINDOW))
	_sampler = FrameSampler.new(warmup, window)
	_saved_max_fps = Engine.max_fps
	Engine.max_fps = 0
	return {"__defer__": true}

## 采样进行中每帧喂真实 delta；采满回 p95 并还原 fps cap。tex_diag 随回包让 A/B 不会认错样本。
func _pump_measure(delta: float) -> void:
	if _sampler == null:
		return
	_sampler.feed(delta)
	if not _sampler.is_done():
		return
	var result := {"ok": true, "op": "measure_p95",
		"p95_ms": _sampler.p95_ms(), "samples": _sampler.sample_count(),
		"tex_diag": Api._tex_diag_downsample}
	_sampler = null
