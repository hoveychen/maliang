...
const BOOK_FILL := 0.82                  ## 跨页宽占屏比
const BOOK_NDC := Vector2(0.0, -0.04)    ## 书中心落点（NDC）
const BOOK_TILT := Vector3(0.34, 0.0, 0.012) ## 基础俯视倾角（顶缘朝相机=看见页缘与沟槽）
...
var _cam: Camera3D
var _bg_quad: MeshInstance3D       ## 天空插画幕布（3D 里铺在书后面）
var _page_root: Control            ## 跨页视口里的页面容器（1040×600 设计系，×1.5 填满视口）
var _sway_t := 0.0                 ## 书的微摆相位（纸片"活着"的感觉）
...
## 3D 舞台：相机 + 卡纸故事书（合书开场）+ 天空幕布。根是 Control，3D 节点挂进
## 主视口的 World3D（2D 画布叠在 3D 之上，本 Control 不画东西只收输入）。
func _setup_stage3d() -> void:
	...
	add_child(_cam)
	_cam.make_current()
	# 天空幕布：bg_onboarding 水彩插画贴大 quad 铺在书后面（2D 画布盖 3D，
	# TextureRect 背景行不通——canvas 永远画在 3D 之上）
	var bg_mesh := QuadMesh.new()
	_bg_quad = MeshInstance3D.new()
	_bg_quad.name = "SkyBackdrop"
	_bg_quad.mesh = bg_mesh
	var bg_mat := StandardMaterial3D.new()
	bg_mat.shading_mode = BaseMaterial3D.SHADING_MODE_UNSHADED
	bg_mat.albedo_texture = UiAssets.tex("bg_onboarding")
	_bg_quad.material_override = bg_mat
	_cam.add_child(_bg_quad)
	# 书：合书姿态入场，翻开封面见 _next_page 首页特例
	_book = PaperBook.new()
...
## 书与天空幕布贴合相机视野（进场与窗口变化时）。
func _fit_stage() -> void:
	_book.fit_to_camera(_cam, BOOK_FILL, BOOK_NDC, CAM_DIST)
	var bg_dist := 8.0
	var h := 2.0 * bg_dist * tan(deg_to_rad(CAM_FOV * 0.5)) * 1.1
	var vs := get_viewport().get_visible_rect().size
	var aspect := (vs.x / vs.y) if vs.y > 1.0 else (16.0 / 9.0)
	(_bg_quad.mesh as QuadMesh).size = Vector2(h * maxf(aspect, 1.6), h)
	_bg_quad.position = Vector3(0.0, 0.0, -bg_dist)
...
func _process(delta: float) -> void:
	# 书本微摆：纸片"活着"的感觉（PaperPhone 持机微摆同源，叠加在基础俯视角上）
	_sway_t += delta
	if _book != null:
		_book.rotation = BOOK_TILT + Vector3(
			sin(_sway_t * 0.7 + 1.3) * 0.006, sin(_sway_t * 0.9) * 0.005, sin(_sway_t * 1.2) * 0.004)
