...
## true = 三个数组来自服务端下发的 .mltr，而非本地 _paint()。离线/回测时恒为 false。
static var _from_server := false

const MLTR_MAGIC := "MLTR"
const MLTR_VERSION := 1
...
##   changed=true → 服务端地形与本地生成的不同。
## changed 很关键：chunk_manager 没有「地形变了重铺全图」的入口，已铺好的区块会留旧样子。
## 步骤①保证导出字节 == _paint() 输出，所以今天 changed 恒为 false；等真有第二张地图时，
## 地形必须在建 chunk_manager 之前就位（见 docs/multi-scene-design.md 步骤⑤）。
static func load_from_bytes(buf: PackedByteArray) -> Dictionary:
	var n := WorldGrid.GRID_TILES
	var count := n * n
	if buf.size() < MLTR_HEADER:
		return { "ok": false, "changed": false, "error": "too short: %d B" % buf.size() }
	for i in range(4):
		if buf[i] != MLTR_MAGIC.unicode_at(i):
			return { "ok": false, "changed": false, "error": "bad magic" }
	if buf[4] != MLTR_VERSION:
		return { "ok": false, "changed": false, "error": "version %d" % buf[4] }
	if buf[5] != n or buf[6] != n:
		return { "ok": false, "changed": false, "error": "grid %dx%d, expect %dx%d" % [buf[5], buf[6], n, n] }
	if buf.size() != MLTR_HEADER + 3 * count:
		return { "ok": false, "changed": false, "error": "length %d" % buf.size() }

	var types := buf.slice(MLTR_HEADER, MLTR_HEADER + count)
	var heights := buf.slice(MLTR_HEADER + count, MLTR_HEADER + 2 * count)
	var depths := buf.slice(MLTR_HEADER + 2 * count, buf.size())
