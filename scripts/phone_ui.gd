...
func _refresh_sticker_shop() -> void:
	...
	for item_id in ItemCatalog.sticker_ids():
		...
		if not ChunkManager.STICKER_TEXTURES.has(key):
			continue
		var cell := VBoxContainer.new()
		cell.alignment = BoxContainer.ALIGNMENT_CENTER
		cell.custom_minimum_size = Vector2(104.0, 0.0)
		var btn := TextureButton.new()
		btn.texture_normal = ChunkManager.STICKER_TEXTURES[key]
...
func refresh_items() -> void:
	...
	for item_id in ids:
		...
		if String(def.get("renderRef", "")).begins_with("sticker:") and ChunkManager.STICKER_TEXTURES.has(skey):
			var tb := TextureButton.new()
			tb.texture_normal = ChunkManager.STICKER_TEXTURES[skey]
