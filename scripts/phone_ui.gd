...
const ITEMS_COLS := 4               ## 背包每行格数
const ITEMS_ROWS := 4               ## 背包每页行数
const ITEMS_PER_PAGE := ITEMS_COLS * ITEMS_ROWS  ## 每页格数（16）
const ITEMS_GLYPH_PX := 128.0       ## 格内图标边长（大格好戳）
...
## 物品货架跨页：4×4 大格背包纵向翻页（backpack-redesign P2）+ 下方贴纸小铺。
## 翻页容器固定单页高（ITEMS_PAGE_H），右侧竖排翻页圆点；数据填充与 snap 见 refresh_items/_step_items_pager。
func _build_items_page() -> Control:
	var page := VBoxContainer.new()
	page.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	page.size_flags_vertical = Control.SIZE_EXPAND_FILL
	page.add_theme_constant_override("separation", 12)
	# 背包区：横排 [纵向翻页容器 | 竖排圆点]。容器高定死一页，多出的物品翻页看。
	...
	_items_pager.size_flags_vertical = Control.SIZE_SHRINK_BEGIN
...
## 建一个背包格：图标（贴纸本尊 / 礼盒占位）+ 名字（份数>1 带 ×N）+ 起名小喇叭角标。
## 点格子进放置模式（组合物走拼装台）。抽成独立函数供 refresh_items 与后续详情面板（P4）复用。
func _make_item_cell(item_id: String, def: Dictionary, count: int) -> Control:
	var cell := VBoxContainer.new()
	cell.alignment = BoxContainer.ALIGNMENT_CENTER
	...
	if stex != null:
		...
		glyph = UiAssets.icon_button("ic_gift", ITEMS_GLYPH_PX) # 点一下进摆放模式（自己选位）
	...
	if String(def.get("renderRef", "")).begins_with("composed:"):
		glyph.pressed.connect(func() -> void: _w._on_composed_item_tapped(item_id))
	else:
		glyph.pressed.connect(func() -> void: _w._begin_placement(item_id))
	...
	if not name_asset.is_empty():
		...
		wrap.custom_minimum_size = Vector2(ITEMS_GLYPH_PX, ITEMS_GLYPH_PX)
