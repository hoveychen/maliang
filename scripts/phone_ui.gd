...
const PHONE_APP_FALLBACK := { "home": "ic_pin", "wishes": "ic_wand", "flowers": "reward_flower", "items": "ic_gift", "stickers": "ic_sparkle", "quiet": "ic_note", "settings": "ic_gear" }
## A4 心愿清单（M1，docs/kids-thinking-little-boss.md）：ability → 「梦想图标」。
## 缺 ability 的卡（chain 跑腿步）用通用目标图标，errand 用带话图标——与委托 chip 的图标语法一致。
const WISH_DREAM_ICON := {
	"create_prop": "ic_gift",
	"create_character": "ic_handshake",
	"create_sticker": "ic_sparkle",
	"play_game": "ic_party",
	"guide_to": "ic_pin",
}
...
var _detail_image: TextureRect      ## 详情大图（缩略图到货时按选中 id 换图；仅在无 3D 回落时用）
var _detail_viewer: Item3DViewer    ## 详情 live 3D 查看器（造物走这个；贴纸/无 3D 回落 _detail_image）
...
# —— A4 心愿清单页（M1） ——
var _wishes_list: VBoxContainer     ## 卡片列（≤3 张：谁在等 + 他梦见的东西）
var _wishes_empty: Label            ## 空态（没人在等也没关系，零催促）
var _wish_cards: Dictionary = {}    ## characterId → 卡片 Control（刷新时 diff：新卡加入、旧卡滑走）
...
func open_app(id: String) -> void:
	...
	if id == "wishes":
		refresh_wishes()  # 开页即按 world 缓存的清单重排（npc_wishes 驱动增删）
...
## ── A4 心愿清单页（M1，docs/kids-thinking-little-boss.md）──────────────────────
## 只读视图：谁在等我 + 他梦见的东西（≤3 张卡）。零倒计时、零催促、不按紧急度排序（§3.1）；
## 挑中谁就走过去搭话，那一刻才变成唯一的 activeTask——幼儿单任务心智与完成判定都不动（§2）。
func _build_wishes_page() -> Control:
	var page := VBoxContainer.new()
	page.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	page.size_flags_vertical = Control.SIZE_EXPAND_FILL
	page.add_theme_constant_override("separation", 18)
	_wishes_list = VBoxContainer.new()
	_wishes_list.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	_wishes_list.add_theme_constant_override("separation", 16)
	page.add_child(_wishes_list)
	_wishes_empty = Label.new()
	# 空态不催不派活：没人在等也完全没关系（§3.1 零压力）
	_wishes_empty.text = "这会儿没有小伙伴在等你\n去村里逛逛吧"
	_wishes_empty.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	_wishes_empty.autowrap_mode = TextServer.AUTOWRAP_ARBITRARY
	_wishes_empty.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	UiAssets.style_card_label(_wishes_empty, 34)
	page.add_child(_wishes_empty)
	return page

## npc_wishes 驱动的增删（world._on_npc_wishes → 这里）：新卡加入、消失的卡带动画退场。
## 完成心愿/跑腿后服务端立刻重推 npc_wishes，所以卡的退场与盖章庆祝同拍（§3.3）。
func refresh_wishes() -> void:
	if _wishes_list == null or _w == null:
		return
	var board: Array = _w.wish_board
	var keep := {}
	for e in board:
		keep[String((e as Dictionary).get("characterId", ""))] = true
	for cid in _wish_cards.keys():
		if not keep.has(cid):
			_dismiss_wish_card(String(cid))
	for i in board.size():
		var e := board[i] as Dictionary
		var cid := String(e.get("characterId", ""))
		if not _wish_cards.has(cid):
			var card := _make_wish_card(e)
			_wish_cards[cid] = card
			_wishes_list.add_child(card)
		_wishes_list.move_child(_wish_cards[cid] as Control, mini(i, _wishes_list.get_child_count() - 1))
	_wishes_empty.visible = _wish_cards.is_empty()

## 卡退场：页面可见才演动画（VBox 布局压 position，改用淡出+缩小），不可见直接拆。
func _dismiss_wish_card(cid: String) -> void:
	var card := _wish_cards.get(cid) as Control
	_wish_cards.erase(cid)
	if card == null:
		return
	var page := _album_pages.get("wishes") as Control
	if page == null or not page.visible or not card.is_inside_tree():
		card.queue_free()
		return
	...
	tw.tween_property(card, "scale", Vector2(0.6, 0.6), 0.35).set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_IN)
	tw.chain().tween_callback(card.queue_free)

## 一张卡 = 村民头像 ⇒ 梦想图标：与委托 chip 同一套图标语法，幼儿零文字可读。
func _make_wish_card(e: Dictionary) -> Control:
	var card := PanelContainer.new()
	card.add_theme_stylebox_override("panel", UiAssets.card_style(18.0, 1.0))
	...
	pad.add_child(row)
	row.add_child(_wish_portrait(String(e.get("characterId", ""))))
	var arrow := Label.new()
	arrow.text = "⇒"
	UiAssets.style_card_label(arrow, 40)
	row.add_child(arrow)
	row.add_child(_wish_icon(_dream_icon_for(e)))
	return card

## 头像：从已降生的角色节点取立绘（同委托 chip 的 portrait 路径）；不在场回落通用目标图标。
func _wish_portrait(cid: String) -> Control:
	var node = _w._find_npc_by_id(cid) if _w != null else null
	if node != null:
		var tex: Texture2D = node.portrait_tex()
		if tex != null:
			var r := TextureRect.new()
			r.texture = tex
			r.expand_mode = TextureRect.EXPAND_IGNORE_SIZE
			r.stretch_mode = TextureRect.STRETCH_KEEP_ASPECT_COVERED
			r.custom_minimum_size = Vector2(96.0, 96.0)
			r.clip_contents = true
			r.mouse_filter = Control.MOUSE_FILTER_IGNORE
			return r
	return _wish_icon("ic_target")

## 梦想图标：ability 命中就用它；chain 跑腿步（无 ability）用目标图标、errand 用带话图标。
func _dream_icon_for(e: Dictionary) -> String:
	var ability := String(e.get("ability", ""))
	if WISH_DREAM_ICON.has(ability):
		return String(WISH_DREAM_ICON[ability])
	return "ic_chat" if String(e.get("source", "")) == "errand" else "ic_target"

func _wish_icon(name: String) -> TextureRect:
	var r := TextureRect.new()
	r.texture = UiAssets.tex(name)
	r.expand_mode = TextureRect.EXPAND_IGNORE_SIZE
	r.stretch_mode = TextureRect.STRETCH_KEEP_ASPECT_CENTERED
	r.custom_minimum_size = Vector2(88.0, 88.0)
	r.mouse_filter = Control.MOUSE_FILTER_IGNORE
	return r
...
func _show_detail_empty(host: VBoxContainer = null) -> void:
	...
	_selected_item = ""
	_detail_image = null
	_detail_viewer = null # 宿主清空即连视口一起 free（下面 queue_free host 子节点）
...
func _select_item(item_id: String, host: VBoxContainer = null) -> void:
	...
	img_box.custom_minimum_size = Vector2(220.0, 220.0)
	img_box.size_flags_horizontal = Control.SIZE_SHRINK_CENTER
	_detail_image = null
	_detail_viewer = null
	# 造物：live 3D 查看器（真模型、自转、可拖动看）。贴纸/无 3D/构建失败 → 回落静态大图。
