...
var _selected_item := ""            ## 当前选中的物品 id（空=未选）
...
func refresh_items() -> void:
	...
	_rebuild_items_dots()
	# 选中的物品若已不在背包（扔了/摆了/用光了），详情面板回空态。
	if _selected_item != "" and not _selected_item in ids and _items_detail != null:
		_show_detail_empty()
...
func _make_item_cell(item_id: String, def: Dictionary, count: int) -> Control:
	...
	glyph.pressed.connect(func() -> void: _select_item(item_id))
...
## 未选中：左半页空态提示（点点小像 + 一句「点一个玩意儿看看」）。
func _show_detail_empty() -> void:
	if _items_detail == null:
		return
	_selected_item = ""
	_detail_image = null
	for c in _items_detail.get_children():
		_items_detail.remove_child(c)
		c.queue_free()
	var icon := UiAssets.icon_rect("ic_gift", 96.0)
	icon.modulate = Color(1.0, 1.0, 1.0, 0.5)
	_items_detail.add_child(icon)
	var hint := Label.new()
	hint.text = "点一个玩意儿看看"
	hint.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	UiAssets.style_card_label(hint, 32)
	_items_detail.add_child(hint)

## 选中一个物品：左半页出大图 + 名字 + 动作按钮竖排（点点念名走 P6）。
func _select_item(item_id: String) -> void:
	if _items_detail == null or item_id.is_empty():
		return
	...
	_items_detail.add_child(_detail_image)
	# 名字
	var name_label := Label.new()
	...
	_items_detail.add_child(name_label)
	# 动作按钮竖排（老板定的集合）。
